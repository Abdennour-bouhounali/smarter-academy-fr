...
export function isMissingSquareRoot(studentVal, expectedVal) {
  const studentNum = parseFloat(studentVal?.toString().replace(',', '.'));
  const expectedNum = parseFloat(expectedVal?.toString().replace(',', '.'));

  if (!isNaN(studentNum) && !isNaN(expectedNum)) {
    // Check if student value is roughly the square of the expected value
    if (Math.abs(studentNum - (expectedNum * expectedNum)) < 0.01) {
      return { message: `Tu as calculé le carré (${studentNum}), mais n'oublie pas d'utiliser la racine carrée pour trouver la longueur finale !` };
    }
  }
  return null;
}
...
export function isSignError(studentVal, expectedVal) {
  ...
  if (!isNaN(studentNum) && !isNaN(expectedNum) && expectedNum !== 0) {

...
export default function Module04EcritureScientifique() {
  ...
  const validateAnswer = (answerObj) => {
    let isCorrect = true;
    let feedback = null;
    const fields = { coef: true, exp: true };
    ...
    if (c !== '4.5') {
      ...
      fields.coef = false;
      const cNum = Number(c);
      if (isNaN(cNum) || c.trim() === '') {
        feedback = "Le coefficient doit être un nombre.";
      } else if (cNum >= 10) {
        feedback = "Attention, le coefficient doit être strictement inférieur à 10 !";
      } else if (cNum < 1) {
        feedback = "Attention, le coefficient doit être supérieur ou égal à 1 !";
      } else {
        feedback = "Le coefficient est incorrect. Où placer la virgule dans 45 000 ?";
      }
    } else if (e !== '4') {
      isCorrect = false;
      fields.exp = false;
      const eNum = Number(e);
      if (isNaN(eNum) || e.trim() === '') {
        feedback = "L'exposant doit être un nombre entier.";
      } else {
        feedback = `Presque ! De combien de rangs la virgule s'est-elle déplacée de 4,5 pour arriver à 45 000 ?`;
      }
    }

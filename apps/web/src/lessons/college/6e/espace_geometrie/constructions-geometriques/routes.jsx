...
const MODULE_COMPONENTS = {
  ...
  1: lazy(() => import('./modules/Module01CroquisRate.jsx')),
  2: lazy(() => import('./modules/Module02RegleGraduee.jsx')),
  3: lazy(() => import('./modules/Module03Compas.jsx')),
  4: lazy(() => import('./modules/Module04Equerre.jsx')),
  5: lazy(() => import('./modules/Module05Programme.jsx')),
  6: lazy(() => import('./modules/Module06Verification.jsx')),
  ...
  return (
    <Suspense fallback={<div className="min-h-screen bg-slate-50" />}>
      <LazyLessonKnowledgeProvider
        ...
        printTitle="LES CONSTRUCTIONS GÉOMÉTRIQUES"
...
export default function constructionsGeometriquesRoutes() {
  return [
    <Route key="constructions-index" path={LESSON_BASE_PATH} element={withSuspense(LessonHome)} />,
    ...LESSON_CONFIG.modules
      .map((m) => {
        const Component = MODULE_COMPONENTS[m.number];
        if (!Component) return null;
        return <Route key={`constructions-${m.id}`} path={m.path} element={withSuspense(Component)} />;
      })
      .filter(Boolean),
  ];
}

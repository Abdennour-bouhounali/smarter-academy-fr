...
const MODULE_COMPONENTS = {
  ...
  1: lazy(() => import('./modules/Module01Ouverture.jsx')),
  2: lazy(() => import('./modules/Module02ComparerClasser.jsx')),
  3: lazy(() => import('./modules/Module03Rapporteur.jsx')),
  4: lazy(() => import('./modules/Module04BonneGraduation.jsx')),
  5: lazy(() => import('./modules/Module05Construire.jsx')),
  6: lazy(() => import('./modules/Module06Missions.jsx')),
...
export default function anglesRoutes() {
  return [
    <Route key="angles-index" path={LESSON_BASE_PATH} element={withSuspense(LessonHome)} />,
    ...LESSON_CONFIG.modules
      .map((m) => {
        const Component = MODULE_COMPONENTS[m.number];
        if (!Component) return null;
        return <Route key={`angles-${m.id}`} path={m.path} element={withSuspense(Component)} />;
      })
      .filter(Boolean),
  ];
}

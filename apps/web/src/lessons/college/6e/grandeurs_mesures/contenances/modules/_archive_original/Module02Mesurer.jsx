...
function ReadRound({ round, done, onSolved }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <ChoiceGrid options={round.options} selected={pick} onSelect={(i) => { setPick(i); setChecked(true); onSolved?.(); }} revealed={checked} correctIndex={round.correct} cols={3} disabled={done} />
      {checked && (
        <Feedback tone={isRight ? 'ok' : 'ko'}>
          {isRight ? 'C’est bien ' : <>Bonne réponse : </>}
...
export default function Module02Mesurer() {
  ...
  const alreadyCompleted = isModuleCompleted('2');
  ...
  const allDone = alreadyCompleted || (refDone && allReadDone);

  const incompleteSteps = [
    !refDone && { num: 1, title: '1 litre, un repère à connaître' },
    !allReadDone && { num: 2, title: 'Lis le niveau' },
  ].filter(Boolean);
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 py-8 flex-1 w-full space-y-8">
        ...
        <StepCard num={2} title="Lis le niveau" done={allReadDone} locked={isStepLocked(alreadyCompleted, !refDone)}>

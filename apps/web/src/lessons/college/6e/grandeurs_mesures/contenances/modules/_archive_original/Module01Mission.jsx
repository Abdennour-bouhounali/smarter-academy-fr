      ...
      {poured && (
        <div className="space-y-4">
          ...
          <ChoiceGrid
            ...
            selected={pick}
            onSelect={(i) => { setPick(i); setChecked(true); onSolved?.(); }}
            revealed={checked}
          ...
          {checked && (
            <Feedback tone={isRight ? 'ok' : 'ko'}>
              {!isRight && (
                <>
                  Bonne réponse : <strong>la bouteille contient plus</strong> — regarde, la cruche a débordé. {' '}
                </>
              )}
...
export default function Module01Mission() {
  ...
  const allDone = alreadyCompleted || (pourDone && freeDone);

  const incompleteSteps = [
    !pourDone && { num: 1, title: 'La bouteille ou la cruche ?' },
    !freeDone && { num: 2, title: 'À toi de transvaser' },
  ].filter(Boolean);
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 py-8 flex-1 w-full space-y-8">
        ...
        <StepCard num={2} title="À toi de transvaser" done={freeDone} locked={isStepLocked(alreadyCompleted, !pourDone)}>

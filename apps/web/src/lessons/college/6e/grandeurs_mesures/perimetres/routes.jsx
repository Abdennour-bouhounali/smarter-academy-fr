...
const MODULE_COMPONENTS = {
  ...
  2: lazy(() => import('./modules/Module02TourComplet.jsx')),
  3: lazy(() => import('./modules/Module03Formules.jsx')),
  4: lazy(() => import('./modules/Module04TourDuCercle.jsx')),
  5: lazy(() => import('./modules/Module05BonsReflexes.jsx')),
  6: lazy(() => import('./modules/Module06Ateliers.jsx')),
...
export default function perimetresRoutes() {
  return [
    <Route key="perimetres-index" path={LESSON_BASE_PATH} element={withSuspense(LessonHome)} />,
    ...LESSON_CONFIG.modules
      .map((m) => {
        const Component = MODULE_COMPONENTS[m.number];
        if (!Component) return null;
        return <Route key={`perimetres-${m.id}`} path={m.path} element={withSuspense(Component)} />;
      })
      .filter(Boolean),
  ];
}

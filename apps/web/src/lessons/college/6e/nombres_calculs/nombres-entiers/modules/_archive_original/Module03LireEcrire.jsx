...
function BigNumberReader({ item, onSolved, solved, react }) {
        ...
        <>
          ...
          {revealed && (
            <Feedback tone={pick === item.correct ? 'ok' : 'ko'}>
              {pick !== item.correct && (
                <>
                  Ta réponse : <strong>{item.options[pick]}</strong>. Bonne réponse :{' '}
...
export default function Module03LireEcrire() {
  ...
  const [formatBurst, setFormatBurst] = useState(0);
  ...
  const incompleteSteps = [
    !s1 && { num: 1, title: 'Des mots vers les chiffres' },
    !s2 && { num: 2, title: 'Des chiffres vers les mots' },
    !s3 && { num: 3, title: 'Lire les grands nombres' },
  ].filter(Boolean);
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 py-8 flex-1 w-full space-y-8">
            ...
            {EN_LETTRES.map((item, i) => (
              <div key={item.target} className="space-y-3">
                ...
                <NumberBuilder
                  ...
                  onSolved={() => {
                    if (!lettresDone.includes(i)) react(true);
                    mark(setLettresDone, lettresDone, i);
                  }}
                />
              </div>
            ))}
        ...
        {/* Étape 2 */}
        <StepCard num={2} title="Des chiffres vers les mots" done={s2} locked={isStepLocked(alreadyCompleted, !s1)}>
          <div className="space-y-6">
              ...
              <div className="space-y-3 border-t border-slate-200 pt-5">
                ...
                <div className="relative">
                  <ChoiceGrid
                    ...
                    onSelect={(i) => {
                      setFormatPick(i);
                      setFormatRevealed(true);
                      const isCorrect = i === FORMAT_Q.correct;
                      const id = react(isCorrect);
                      if (isCorrect) setFormatBurst(id);
                    }}
                    revealed={formatRevealed}
                    correctIndex={FORMAT_Q.correct}
                    cols={3}
                  />
                  <XPBurst amount={10} tick={formatBurst} />
                </div>
                {formatRevealed && (
...
/* ─── Item « chiffres → mots » ───────────────────────────────────── */
function ReverseItem({ item, solved, onSolved, react }) {
  const [pick, setPick] = useState(null);
  const [revealed, setRevealed] = useState(false);
  const [burst, setBurst] = useState(0);

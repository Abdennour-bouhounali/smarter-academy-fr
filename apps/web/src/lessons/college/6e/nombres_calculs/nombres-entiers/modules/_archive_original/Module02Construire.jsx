...
import {
  Feedback, StepCard, MissionBrief,
  StepProgressBar, StreakChip, EffectsToggle,
...
export default function Module02Construire() {
  ...
  const allDone = alreadyCompleted || (s1 && s2 && s3a && s3b);
  const doneCount = [s1, s2, s3a && s3b].filter(Boolean).length;

  const incompleteSteps = [
    !s1 && { num: 1, title: 'Construis le nombre 347' },
    !s2 && { num: 2, title: 'Construis maintenant 1 205' },
    !(s3a && s3b) && { num: 3, title: "Atelier d'échange : de 37 à 50" },
  ].filter(Boolean);

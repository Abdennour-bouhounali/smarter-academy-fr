...
function Question({ item, solved, onSolved, index, total }) {
  ...
  const checkInput = () => {
    ...
    if (n === item.answer) {
      ...
      setWrong(item.wrongHint?.(n) || item.hint);
    }
  };
...
function NombreMystere({ solved, onSolved }) {
  const [val, setVal] = useState('');
    ...
    <div className="space-y-4">
      ...
      {solved ? (
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-3">
          <Feedback tone="ok">
            Le nombre mystère est <strong className="font-mono">{formatFr(MYSTERE)}</strong>. Chaque indice
            remplissait une colonne du tableau de numération.
          </Feedback>
  ...
  return (
    ...
    >
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        {/* Problème 2 */}
        <StepCard num={2} title="Problème 2 — Les trois communes" subtitle="Niveau : intermédiaire" done={s2} locked={!s1}>
          <div className="space-y-4">
        ...
        {/* Problème 3 */}
        <StepCard num={3} title="Problème 3 — Le nombre mystère" subtitle="Niveau : expert" done={s3} locked={!s2}>
          <div className="space-y-5">
            ...
            <NombreMystere solved={mystereDone} onSolved={() => setMystereDone(true)} />

...
import {
  Feedback, ChoiceGrid, StepCard, MissionBrief, QuantityCard,
...
export default function Module09MondeReel() {
  ...
  const alreadyCompleted = isModuleCompleted('9');
  ...
  const [manqBurst, setManqBurst] = useState(0);

  const carteCorrect = pickCarte === PLUS_GRAND;
  const s1 = carteRevealed;
  const s2 = estimDone.length === ESTIMATIONS.length;
  const s3 = manqRevealed;
  const allDone = alreadyCompleted || (s1 && s2 && s3);
  const doneCount = [s1, s2, s3].filter(Boolean).length;

  const incompleteSteps = [
    !s1 && { num: 1, title: 'Quel nombre représente la plus grande quantité ?' },
    !s2 && { num: 2, title: 'Estime avant de lire la valeur exacte' },
    !s3 && { num: 3, title: 'Quel nombre manque ?' },
  ].filter(Boolean);
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 flex-1 w-full space-y-8">
        ...
        <StepCard num={1} title="Quel nombre représente la plus grande quantité ?" done={s1}>
          <div className="space-y-4">
              ...
              {CARTES.map((c, i) => (
                ...
                >
                  <QuantityCard
                    ...
                    disabled={carteRevealed}
                    onClick={() => {
                      if (carteRevealed) return;
                      setPickCarte(c.id);
                      setCarteRevealed(true);
                      react(c.id === PLUS_GRAND);
                    }}
                    footer={
                      carteRevealed ? (
            ...
            {carteRevealed && (
              <Feedback tone={carteCorrect ? 'ok' : 'ko'}>
                {carteCorrect ? (
                  <>
                  ...
                  <>
                    Bonne réponse : <strong>la distance Terre – Lune</strong>. Commence par repérer les nombres
                    qui ont <strong>le plus de chiffres</strong> : 105 300, 384 400 et 305 000 en ont 6. Ensuite,
                    compare-les position par position.
                  </>
                )}
              </Feedback>
            )}
        ...
        {/* Étape 3 */}
        <StepCard num={3} title="Quel nombre manque ?" done={s3} locked={isStepLocked(alreadyCompleted, !s2)}>
          <div className="space-y-4">
            ...
            <div className="relative">
              <ChoiceGrid
                ...
                onSelect={(i) => {
                  setManqPick(i);
                  setManqRevealed(true);
                  const isCorrect = i === MANQUANT.correct;
                  const id = react(isCorrect);
                  if (isCorrect) setManqBurst(id);
                }}
                revealed={manqRevealed}
                correctIndex={MANQUANT.correct}
                cols={2}
              />
              <XPBurst amount={10} tick={manqBurst} />
            </div>
            {manqRevealed && (
              <Feedback tone={manqPick === MANQUANT.correct ? 'ok' : 'ko'}>
                {manqPick !== MANQUANT.correct && (
                  <>
                    Bonne réponse : <strong>{formatFr(MANQUANT.options[MANQUANT.correct])}</strong>. {' '}
                  </>
                )}
                {MANQUANT.explain}
              </Feedback>
...
/* ─── Item d'estimation ──────────────────────────────────────────── */
function EstimationItem({ item, solved, onSolved, react }) {

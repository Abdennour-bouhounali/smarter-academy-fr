...
export default function Module07RangerEncadrer() {
  ...
  const alreadyCompleted = isModuleCompleted('7');
  ...
  const [consoBurst, setConsoBurst] = useState(0);

  const s3 = niveauxDone.length === NIVEAUX.length && consoRevealed;
  const allDone = alreadyCompleted || (ascDone && descDone && s3);
  const doneCount = [ascDone, descDone, s3].filter(Boolean).length;

  const incompleteSteps = [
    !ascDone && { num: 1, title: "Range dans l'ordre croissant" },
    !descDone && { num: 2, title: "Maintenant dans l'ordre décroissant" },
    !s3 && { num: 3, title: 'Encadrer 4 582 : de plus en plus précis' },
  ].filter(Boolean);
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 py-8 flex-1 w-full space-y-8">
        ...
        <StepCard num={2} title="Maintenant dans l'ordre décroissant" done={descDone} locked={isStepLocked(alreadyCompleted, !ascDone)}>
          <OrderingGame
            items={A_RANGER}
            direction="desc"
            solved={descDone}
        ...
        <StepCard
          ...
          locked={isStepLocked(alreadyCompleted, !descDone)}
        >
              ...
              <div className="space-y-4 border-t border-slate-200 pt-5">
                ...
                <div className="relative">
                  <ChoiceGrid
                    ...
                    onSelect={(i) => {
                      setConsoPick(i);
                      setConsoRevealed(true);
                      const isCorrect = i === CONSO.correct;
                      const id = react(isCorrect);
                      if (isCorrect) setConsoBurst(id);
                    }}
                    revealed={consoRevealed}
                    correctIndex={CONSO.correct}
                    cols={2}
                  />
                  <XPBurst amount={10} tick={consoBurst} />
                </div>
                {consoRevealed && (
                  <Feedback tone={consoPick === CONSO.correct ? 'ok' : 'ko'}>
                    {consoPick !== CONSO.correct && (
                      <>
                        Bonne réponse : <strong>{CONSO.options[CONSO.correct]}</strong>. {' '}
                      </>
                    )}
                    {CONSO.explain}
                  </Feedback>

...
export default function Module01Mission() {
  ...
  const alreadyCompleted = isModuleCompleted('1');
  ...
  const [reflexionBurst, setReflexionBurst] = useState(0);

  const [piegePick, setPiegePick] = useState(null);
  const [piegeRevealed, setPiegeRevealed] = useState(false);
  const [piegeBurst, setPiegeBurst] = useState(0);

  const step2Done = reflexionRevealed;
  const step3Done = piegeRevealed;
  const allDone = alreadyCompleted || (step1Done && step2Done && step3Done);
  const doneCount = [step1Done, step2Done, step3Done].filter(Boolean).length;

  const incompleteSteps = [
    !step1Done && { num: 1, title: 'Range les six quantités, de la plus petite à la plus grande' },
    !step2Done && { num: 2, title: "Qu'est-ce qui t'a aidé à décider ?" },
    !step3Done && { num: 3, title: 'Un dernier duel avant de partir' },
  ].filter(Boolean);
    ...
    >
      <div className="max-w-7xl mx-auto px-4 py-8 flex-1 w-full space-y-8">
        ...
        <StepCard
          ...
          locked={isStepLocked(alreadyCompleted, !step1Done)}
        >
          ...
          <div className="relative">
            <ChoiceGrid
              ...
              onSelect={(i) => {
                setReflexionPick(i);
                setReflexionRevealed(true);
                const isCorrect = i === REFLEXION.correct;
                const id = react(isCorrect);
                if (isCorrect) setReflexionBurst(id);
              }}
              revealed={reflexionRevealed}
              correctIndex={REFLEXION.correct}
            />
            <XPBurst amount={10} tick={reflexionBurst} />
          </div>
          {reflexionRevealed && (
        ...
        <StepCard
          ...
          locked={isStepLocked(alreadyCompleted, !step2Done)}
        >
          ...
          <div className="relative">
            <ChoiceGrid
              ...
              onSelect={(i) => {
                ...
                const isCorrect = i === PIEGE.correct;
                const id = react(isCorrect);
                if (isCorrect) setPiegeBurst(id);
              }}
              revealed={piegeRevealed}
              correctIndex={PIEGE.correct}
              cols={3}
            />
            <XPBurst amount={10} tick={piegeBurst} />
          </div>

...
function MultiDecomposition({ onSolved, solved, react }) {
                ...
                {[
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      setAnswers(next);
                      // Dernière réponse posée → correction immédiate, sans bouton
                      if (Object.keys(next).length === PROPOSITIONS.length) {
                        setChecked(true);
                        const allR = PROPOSITIONS.every((p, j) => next[j] === (p.value === CIBLE));
                        react(allR);
                        onSolved?.();
                      }
                    }}
...
function RecomposeItem({ item, solved, onSolved, react }) {
...
export default function Module05Decomposer() {
  ...
  const [zeroBurst, setZeroBurst] = useState(0);
  ...
  const incompleteSteps = [
    !s1 && { num: 1, title: 'Décompose 4 582' },
    !s2 && { num: 2, title: 'Y a-t-il une seule bonne décomposition ?' },
    !s3 && { num: 3, title: 'Recompose : de la somme au nombre' },
    !s4 && { num: 4, title: 'Défi : le zéro, gardien des positions' },
  ].filter(Boolean);
    ...
    >
      <div className="max-w-7xl mx-auto px-4 py-8 flex-1 w-full space-y-8">
        ...
        <StepCard num={1} title="Décompose 4 582" subtitle="Choisis les bonnes tuiles — attention aux pièges." done={s1}>
          <DecompositionTiles target={CIBLE} solved={s1} onSolved={() => setS1(true)} react={react} />
        </StepCard>
        ...
        >
          <MultiDecomposition solved={s2} onSolved={() => setS2(true)} react={react} />
        </StepCard>
                ...
                <div className="relative">
                  <ChoiceGrid
                    ...
                    onSelect={(i) => {
                      ...
                      const isCorrect = i === ZERO_QUESTION.correct;
                      const id = react(isCorrect);
                      if (isCorrect) setZeroBurst(id);
                    }}
                    revealed={zeroRevealed}
                    correctIndex={ZERO_QUESTION.correct}
                    cols={1}
                  />
                  <XPBurst amount={10} tick={zeroBurst} />
                </div>
                {zeroRevealed && (
                    ...
                    <div className="relative">
                      <ChoiceGrid
                        ...
                        onSelect={(i) => {
                          setErrPick(i);
                          setErrRevealed(true);
                          const isCorrect = i === ERREUR_ZERO.correct;
                          const id = react(isCorrect);
                          if (isCorrect) setErrBurst(id);
                        }}
                        revealed={errRevealed}
                        correctIndex={ERREUR_ZERO.correct}
                        cols={1}
                      />
                      <XPBurst amount={10} tick={errBurst} />
                    </div>
                    {errRevealed && (

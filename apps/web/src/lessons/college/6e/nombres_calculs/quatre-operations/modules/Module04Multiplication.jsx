...
/* ─── Manipulation 1 : Groupes égaux ──────────────────────────── */
function EqualGroups({ onComplete, done }) {
      ...
      <AnimatePresence>
        {finished && !done && (
          <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="space-y-3">
            ...
            <button onClick={onComplete} className="w-full py-3 bg-violet-600 hover:bg-violet-700 text-white font-bold rounded-xl transition-all">
              Continuer →
            </button>
          </motion.div>
        )}
...
/* ─── Manipulation 2 : Grille interactive ─────────────────────── */
function GridManip({ onComplete, done }) {
  ...
  return (
    <div className="space-y-5">
      ...
      {!done && !validated && (
        <button onClick={() => { setValidated(true); onComplete(); }} className="w-full py-3 bg-violet-600 hover:bg-violet-700 text-white font-bold rounded-xl transition-all">
...
/* ─── Manipulation 3 : Décomposition ─────────────────────────── */
function DecompositionManip({ onComplete, done }) {
...
/* ─── Multiplication posée : 23 × 14 ────────────────────────── */
function PosedMultiplication({ onComplete, done }) {
  const [step, setStep] = useState(0);
...
function MultPractice({ onComplete, done }) {
  const exercises = [
    { q: '6 × 7', ans: 42 },
    { q: '8 × 9', ans: 72, hint: 'Tu peux décomposer : 8×9 = 8×10 − 8×1' },
    { q: '4 × 25', ans: 100, hint: '4 × 25 = 4 × (20 + 5) = 80 + 20 = 100' },
  ];
  ...
  const check = () => {
    if (parseInt(val) === exercises[idx].ans) setFb('ok');
    else setFb('ko');
  };
  const next = () => {
    if (idx < exercises.length - 1) { setIdx((i) => i + 1); setVal(''); setFb(null); }
    else onComplete();
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between text-xs font-mono text-slate-400">
        <span>Exercice {idx + 1}/{exercises.length}</span>
      </div>
      <div className="bg-slate-800 text-white rounded-xl p-5 text-center text-3xl font-space font-bold">{ex.q} = ?</div>
      <div className="flex gap-3">
        <input type="number" value={val} onChange={(e) => { setVal(e.target.value); setFb(null); }} onKeyDown={(e) => { if (e.key === 'Enter') check(); }}
          placeholder="?" className="flex-1 border-2 border-slate-200 rounded-xl px-4 py-3 text-xl font-mono text-center focus:outline-none focus:border-violet-400" />
        <button onClick={check} disabled={!val || fb === 'ok'} className="px-6 py-3 bg-violet-600 hover:bg-violet-700 text-white font-bold rounded-xl disabled:opacity-40">OK</button>
      </div>
        ...
        {fb === 'ok' && (
          <motion.div key="ok" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-2">
            <div className="text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3 text-sm">
              <CheckCircle2 className="inline w-4 h-4 mr-1" /> <strong>{ex.q} = {ex.ans}</strong> ✓
            </div>
            <button onClick={next} className="w-full py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl text-sm">
              {idx < exercises.length - 1 ? 'Suivant →' : 'Terminer →'}
            </button>
          </motion.div>
        )}
        {fb === 'ko' && (
          <motion.div key="ko" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-2">
            <div className="text-rose-700 bg-rose-50 border border-rose-200 rounded-xl px-4 py-3 text-sm">
              <XCircle className="inline w-4 h-4 mr-1" /> Pas tout à fait.
              {ex.hint && <span> 💡 {ex.hint}</span>}
            </div>
            <button onClick={() => setFb(null)} className="text-sm text-slate-500 underline">Réessayer</button>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
    ...
    >
      <div className="max-w-3xl mx-auto space-y-6">
        <div className="flex gap-1.5 flex-wrap">
          {['Groupes égaux', 'Grille', 'Décomposition', 'Posée', 'Pratique'].map((l, i) => (
            <div key={i} className={`px-3 py-1 rounded-full text-[11px] font-mono font-bold transition-all ${i < phase ? 'bg-emerald-100 text-emerald-700' : i === phase ? 'bg-violet-600 text-white' : 'bg-slate-100 text-slate-400'}`}>
              {i < phase ? '✓ ' : ''}{l}
            </div>
          ))}
        </div>
        ...
        {phase === 0 && (
          <div className="bg-white border border-slate-200 rounded-2xl p-6 space-y-4">
            <h2 className="text-lg font-space font-bold text-slate-800">Des groupes de même taille</h2>
            <EqualGroups onComplete={() => setPhase(1)} done={phase > 0} />
          </div>
        )}
        ...
        {phase >= 1 && (
          <div className="bg-white border border-slate-200 rounded-2xl p-6 space-y-4">
            <h2 className="text-lg font-space font-bold text-slate-800">La grille rectangulaire</h2>
            <GridManip onComplete={() => setPhase(2)} done={phase > 1} />
          </div>
        )}
        ...
        {phase >= 2 && (
          <div className="bg-white border border-slate-200 rounded-2xl p-6 space-y-4">
            <h2 className="text-lg font-space font-bold text-slate-800">Décomposer pour calculer</h2>
            <DecompositionManip onComplete={() => setPhase(3)} done={phase > 2} />
          </div>
        )}
        ...
        {phase >= 3 && (
          <div className="bg-white border border-slate-200 rounded-2xl p-6 space-y-4">
            <h2 className="text-lg font-space font-bold text-slate-800">La multiplication posée</h2>
            <PosedMultiplication onComplete={() => setPhase(4)} done={phase > 3} />
          </div>
        )}
        ...
        {phase >= 4 && (
          <div className="bg-white border border-slate-200 rounded-2xl p-6 space-y-4">
            ...
            <MultPractice onComplete={() => setPhase(5)} done={phase >= 5} />
          </div>
        )}

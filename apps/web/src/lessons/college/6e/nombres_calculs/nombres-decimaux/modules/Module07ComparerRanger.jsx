...
/* ─── Étape 1 : comparaison visuelle simple ──────────────────────── */
const VISUEL = { a: 2.7, b: 2.4 };

/* ─── Étape 2 : le cas critique 2,37 vs 2,4 ──────────────────────── */
...
const CRITIQUE_Q = {
  ...
  correct: 1,
  explain:
    "2,4 est le plus grand. Le piège consiste à comparer 37 et 4 comme des entiers — mais 37 compte des CENTIÈMES et 4 compte des DIXIÈMES : ce ne sont pas les mêmes parts.",
};
...
const ALIGN_Q = {
  ...
  correct: 1,
  explain:
    "On ajoute un zéro À LA FIN : 2,4 = 2,40. La quantité ne change pas, mais on peut maintenant comparer des parts de même taille : 37 centièmes contre 40 centièmes.",
};

/* ─── Étape 3 : laboratoire de comparaison ───────────────────────── */
const LABO = { a: 5.284, b: 5.248 };

function DecCompareLab({ a, b, solved, onSolved }) {
    ...
    <div className="space-y-4">
      <div className="overflow-x-auto">
        <table className="mx-auto border-separate border-spacing-1 min-w-max">
            ...
            <tr>
              ...
              {cellsA.map((c, i) => (
                <React.Fragment key={c.key}>
                  ...
                  <th className="px-1 pb-1 text-[9px] sm:text-[10px] font-mono text-slate-500 font-semibold uppercase">
                    {c.label}
                  </th>
                </React.Fragment>
              ))}
          ...
          <tbody>
            {[
              { label: 'A', cells: cellsA },
              { label: 'B', cells: cellsB },
            ].map((row, ri) => (
                      ...
                      <React.Fragment key={c.key}>
                        {i === intCount && (
                          <td className="text-center align-middle">
                            <span className="text-2xl font-extrabold text-rose-500" aria-hidden="true">,</span>
                          </td>
                        )}
                        <td className="p-0.5">
                          <div
                            className={`w-10 h-11 sm:w-12 sm:h-12 mx-auto rounded-xl border-2 flex items-center justify-center font-mono font-extrabold text-lg sm:text-xl tabular-nums transition-all ${
                              isDecisive
      ...
      {stopped && (
        <div className="space-y-4">
          ...
          {checked && pick !== correctSymbol && (
            <Feedback tone="ko">
              Reprends la première différence : {cellsA[firstDiff].digit} contre {cellsB[firstDiff].digit} aux{' '}
          ...
          {solved && (
            <Feedback tone="ok">
              <span className="font-mono font-bold">
                {formatDec(a)} {correctSymbol} {formatDec(b)}
              </span>{' '}
              — tout s'est joué aux {cellsA[firstDiff].label.toLowerCase()}.
            </Feedback>
          )}
        </div>
      )}
...
/* ─── Étape 4 : raisonnements faux ───────────────────────────────── */
const ERREURS = [
  {
    claim: '« 4,8 < 4,35 car 8 < 35 »',
    options: [
      'On compare 8 dixièmes avec 35 centièmes : il faut d\'abord écrire 4,8 = 4,80, puis comparer 80 et 35 centièmes.',
      'Le raisonnement est correct.',
      'Il fallait comparer le nombre de chiffres après la virgule.',
      'Il fallait comparer les parties entières, qui sont différentes.',
    ],
    correct: 0,
    truth: '4,8 > 4,35',
    detail: '4,8 = 4,80. Or 80 centièmes > 35 centièmes, donc 4,8 > 4,35.',
  },
  {
    claim: '« 3,09 > 3,9 parce que 09 > 9 »',
    options: [
      'Le raisonnement est correct.',
      "On ne peut pas comparer « 09 » et « 9 » comme des entiers : il faut aligner, 3,9 = 3,90, puis comparer 09 et 90 centièmes.",
      'Il fallait ajouter les chiffres après la virgule.',
      'Il fallait comparer les parties entières.',
    ],
    correct: 1,
    truth: '3,09 < 3,90',
    detail: '3,9 = 3,90. Or 9 centièmes < 90 centièmes, donc 3,09 < 3,9.',
  },
  {
    claim: '« 5,40 > 5,4 car 40 > 4 »',
    options: [
      'Le raisonnement est correct.',
      'Non : le zéro est ajouté à la fin, donc 5,40 = 5,4. Ce sont deux écritures du même nombre.',
      'Non : 5,40 est plus petit que 5,4.',
      'Non : il faut comparer les parties entières.',
    ],
    correct: 1,
    truth: '5,40 = 5,4',
    detail: '40 centièmes = 4 dixièmes : la quantité est identique, seule l\'écriture change.',
  },
  {
    claim: '« 2,105 = 2,15 »',
    options: [
      'Le raisonnement est correct.',
      "Non : dans 2,105 le 1 est aux dixièmes, le 0 aux centièmes et le 5 aux millièmes ; dans 2,15 le 5 est aux centièmes. Les colonnes ne correspondent pas.",
      'Non : 2,105 est plus grand que 2,15.',
      'Non : les deux nombres sont plus petits que 2.',
    ],
    correct: 1,
    truth: '2,105 < 2,150',
    detail:
      'En alignant : 2,105 et 2,150. On compare 105 millièmes contre 150 millièmes, donc 2,105 < 2,15.',
  },
];
...
export default function Module07ComparerRanger() {
  ...
  const [visPick, setVisPick] = useState(null);
  const [visRevealed, setVisRevealed] = useState(false);
  const [critPick, setCritPick] = useState(null);
  const [critRevealed, setCritRevealed] = useState(false);
  const [alignPick, setAlignPick] = useState(null);
  const [alignRevealed, setAlignRevealed] = useState(false);
  ...
  const s1 = visRevealed && visPick === 0;
  const s2 = critRevealed && critPick === CRITIQUE_Q.correct && alignRevealed && alignPick === ALIGN_Q.correct;
  const s3 = laboDone;
  const s4 = erreursDone.length === ERREURS.length;
  const s5 = rangeDone;
  const allDone = s1 && s2 && s3 && s4 && s5;
  ...
  return (
    ...
    >
      <div className="max-w-3xl mx-auto space-y-6">
        <MissionBrief tag="⚖️ Comparaison" title="Comparer des décimaux, ce n'est pas comparer des entiers.">
          <p>
        ...
        {/* Étape 1 */}
        <StepCard num={1} title="D'abord avec les yeux" done={s1}>
          <div className="space-y-4">
            <div className="bg-white border-2 border-slate-200 rounded-2xl p-2">
              <NumberLine
                ...
                markers={[
                  { value: VISUEL.b, label: '2,4', color: '#dc2626' },
                  { value: VISUEL.a, label: '2,7', color: '#059669' },
                ]}
                ariaLabel="2,4 et 2,7 placés entre 2 et 3"
              />
            </div>
            <p className="text-sm font-semibold text-slate-700">Lequel est le plus grand ?</p>
            <ChoiceGrid
              options={['2,7', '2,4', 'Ils sont égaux']}
              selected={visPick}
              onSelect={setVisPick}
              revealed={visRevealed}
              correctIndex={0}
              cols={3}
            />
            {!visRevealed && (
              <ValidateButton onClick={() => setVisRevealed(true)} disabled={visPick === null}>
                Valider
              </ValidateButton>
            )}
            {visRevealed && (
        ...
        >
          <div className="space-y-4">
            ...
            <p className="text-sm font-semibold text-slate-700">{CRITIQUE_Q.q}</p>
            <ChoiceGrid
              options={CRITIQUE_Q.options}
              selected={critPick}
              onSelect={setCritPick}
              revealed={critRevealed}
              correctIndex={CRITIQUE_Q.correct}
              cols={3}
            />
            {!critRevealed && (
              <ValidateButton onClick={() => setCritRevealed(true)} disabled={critPick === null}>
                Valider
              </ValidateButton>
            )}
            {critRevealed && (
              <Feedback tone={critPick === CRITIQUE_Q.correct ? 'ok' : 'ko'}>
                {CRITIQUE_Q.explain}
                {critPick !== CRITIQUE_Q.correct && (
                  <>
                    {' '}
                    <button
                      type="button"
                      onClick={() => {
                        setCritRevealed(false);
                        setCritPick(null);
                      }}
                      className="underline font-semibold"
                    >
            ...
            {critRevealed && critPick === CRITIQUE_Q.correct && (
              <>
                <div className="border-t border-slate-100 pt-4 space-y-3">
                  <p className="text-sm font-semibold text-slate-700">{ALIGN_Q.q}</p>
                  <ChoiceGrid
                    options={ALIGN_Q.options}
                    selected={alignPick}
                    onSelect={setAlignPick}
                    revealed={alignRevealed}
                    correctIndex={ALIGN_Q.correct}
                    cols={2}
                  />
                  {!alignRevealed && (
                    <ValidateButton onClick={() => setAlignRevealed(true)} disabled={alignPick === null}>
                      Valider
                    </ValidateButton>
                  )}
                  {alignRevealed && (
                    <Feedback tone={alignPick === ALIGN_Q.correct ? 'ok' : 'ko'}>
                      {ALIGN_Q.explain}
                      {alignPick !== ALIGN_Q.correct && (
                        <>
                          {' '}
                          <button
                            type="button"
                            onClick={() => {
                              setAlignRevealed(false);
                              setAlignPick(null);
                            }}
                            className="underline font-semibold"
                          >
                ...
                {s2 && (
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-3">
                    <div className="grid grid-cols-2 gap-3">
                      {[
                        { txt: align.aText, cents: align.aUnits - 200, tone: 'rose' },
                        { txt: align.bText, cents: align.bUnits - 200, tone: 'emerald' },
                      ].map((x) => (
        ...
        <StepCard
          ...
          locked={!s2}
        >
          <DecCompareLab a={LABO.a} b={LABO.b} solved={laboDone} onSolved={() => setLaboDone(true)} />
          {s3 && (
            <Feedback tone="info">
              La méthode complète : <strong>1.</strong> comparer les parties entières ; <strong>2.</strong> en cas
              d'égalité, comparer les décimales de gauche à droite ; <strong>3.</strong> s'arrêter à la première
              différence. (On peut aussi aligner avec des zéros, comme à l'étape 2 : les deux méthodes donnent le
              même résultat.)
            </Feedback>
          )}
        </StepCard>
            ...
            {ERREURS.map((e, i) =>
              i === 0 || erreursDone.includes(i - 1) ? (
                <ErreurCard
                  ...
                  onSolved={() => setErreursDone((d) => (d.includes(i) ? d : [...d, i]))}
                />
              ) : null
            )}
          </div>
        ...
        >
          ...
          {s5 && (
            <Feedback tone="info">
              2,05 &lt; 2,15 &lt; 2,5 = 2,50 &lt; 2,55. Les cartes <strong>2,5</strong> et{' '}
              <strong>2,50</strong> représentent la même quantité : leur ordre entre elles n'a pas d'importance.
            </Feedback>
          )}
        </StepCard>
      </div>
...
/* ─── Carte d'erreur à diagnostiquer ─────────────────────────────── */
function ErreurCard({ item, solved, onSolved }) {
  ...
  return (
    <div className="space-y-3 border-2 border-amber-200 bg-amber-50/50 rounded-2xl p-4">
      ...
      <p className="text-sm font-semibold text-slate-600">Où est l'erreur ?</p>
      <ChoiceGrid
      ...
      {revealed && (
        <Feedback tone={pick === item.correct ? 'ok' : 'ko'}>
          La bonne réponse est <strong className="font-mono">{item.truth}</strong>. {item.detail}

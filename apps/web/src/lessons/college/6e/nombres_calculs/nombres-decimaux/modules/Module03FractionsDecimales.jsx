import React, { useState } from 'react';
import { motion } from 'framer-motion';
import { Plus, Minus } from 'lucide-react';
import ModuleLayout from '../../../../../common/components/ModuleLayout';
import MathText from '../../../../../common/components/MathText';
import { Feedback, ChoiceGrid, ValidateButton, StepCard, MissionBrief } from '../../../../../common/components/LessonUI';
import { MODULE_CTX, getNavLinks } from '../moduleContext';
import UnitGrid, { QuantityView } from '../components/UnitGrid';

/**
 * Saisie structurée d'une fraction décimale : compteur pour le numérateur,
 * choix explicite du dénominateur parmi 10 / 100 / 1 000.
 * (L'élève ne tape jamais « 37/100 » au clavier comme du texte brut.)
 */
function FractionBuilder({ target, den: targetDen, onSolved, solved, hint }) {
  const [num, setNum] = useState(0);
  const [den, setDen] = useState(null);
  const [checked, setChecked] = useState(false);

  const isRight = num === target && den === targetDen;

  return (
    <div className="space-y-4">
      <div className="flex items-center justify-center gap-6 flex-wrap">
        {/* Numérateur */}
        <div className="text-center space-y-1">
          <div className="text-[10px] font-mono font-bold uppercase tracking-wider text-slate-400">
            Numérateur
          </div>
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={() => {
                setChecked(false);
                setNum((n) => Math.max(0, n - 1));
              }}
              disabled={solved}
              aria-label="Diminuer le numérateur"
              className="w-9 h-9 rounded-lg bg-white border-2 border-slate-200 flex items-center justify-center hover:border-slate-400 disabled:opacity-30 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
            >
              <Minus className="w-4 h-4" aria-hidden="true" />
            </button>
            <span className="font-mono font-extrabold text-2xl tabular-nums w-14 text-center" aria-live="polite">
              {num}
            </span>
            <button
              type="button"
              onClick={() => {
                setChecked(false);
                setNum((n) => n + 1);
              }}
              disabled={solved}
              aria-label="Augmenter le numérateur"
              className="w-9 h-9 rounded-lg bg-white border-2 border-slate-200 flex items-center justify-center hover:border-slate-400 disabled:opacity-30 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
            >
              <Plus className="w-4 h-4" aria-hidden="true" />
            </button>
          </div>
        </div>

        {/* Barre de fraction */}
        <div className="text-center">
          <div className="h-1 w-24 bg-slate-800 rounded-full my-2" aria-hidden="true" />
        </div>

        {/* Dénominateur */}
        <div className="text-center space-y-1">
          <div className="text-[10px] font-mono font-bold uppercase tracking-wider text-slate-400">
            Dénominateur
          </div>
          <div className="flex gap-1.5">
            {[10, 100, 1000].map((d) => (
              <button
                key={d}
                type="button"
                onClick={() => {
                  setChecked(false);
                  setDen(d);
                }}
                disabled={solved}
                aria-pressed={den === d}
                className={`px-3 py-2 rounded-lg border-2 font-mono text-sm font-bold min-h-[40px] transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 ${
                  den === d
                    ? 'bg-blue-600 border-blue-700 text-white'
                    : 'bg-white border-slate-200 text-slate-600 hover:border-blue-400'
                }`}
              >
                {d === 1000 ? '1 000' : d}
              </button>
            ))}
          </div>
        </div>
      </div>

      {/* Aperçu de la fraction construite */}
      <div className="text-center text-2xl text-slate-800 min-h-[48px] flex items-center justify-center">
        {(num > 0 || den !== null) && (
          <MathText>{`$\\frac{${num}}{${den || '?'}}$`}</MathText>
        )}
      </div>

      {!solved && (
        <div className="text-center">
          <ValidateButton
            onClick={() => {
              setChecked(true);
              if (isRight) onSolved?.();
            }}
            disabled={den === null}
          >
            Valider ma fraction
          </ValidateButton>
        </div>
      )}

      {checked && !isRight && (
        <Feedback tone="hint">
          {den !== targetDen
            ? `Regarde en combien de parts égales l'unité est partagée : c'est ce nombre qui donne le dénominateur.`
            : hint || 'Compte à nouveau les parts coloriées : c\'est le numérateur.'}
        </Feedback>
      )}

      {solved && (
        <Feedback tone="ok">
          <MathText>{`$\\frac{${target}}{${targetDen}}$`}</MathText> : {target} part
          {target > 1 ? 's' : ''} sur les {targetDen === 1000 ? '1 000' : targetDen}.
        </Feedback>
      )}
    </div>
  );
}

/* ─── Étape 1 : lire une fraction décimale sur un dessin ─────────── */
const LECTURES = [
  { parts: 10, shaded: 3, tone: 'sky', name: '3 dixièmes' },
  { parts: 10, shaded: 7, tone: 'sky', name: '7 dixièmes' },
  { parts: 100, shaded: 25, tone: 'violet', name: '25 centièmes' },
  { parts: 100, shaded: 37, tone: 'violet', name: '37 centièmes' },
];

/* ─── Étape 2 : une fraction plus grande que 1 ───────────────────── */
const PLUS_DE_UN = {
  q: 'On te donne 405/100. Que représente cette quantité ?',
  options: [
    "Un peu plus de 4 unités entières",
    'Un peu moins de 1 unité',
    'Exactement 405 unités',
    'Un peu plus de 40 unités',
  ],
  correct: 0,
  explain:
    '100 centièmes forment 1 unité. Dans 405 centièmes, il y a 4 paquets de 100 (soit 4 unités entières) et il reste 5 centièmes. La quantité vaut donc 4 unités et 5 centièmes.',
};

/* ─── Étape 3 : décomposer une fraction décimale ─────────────────── */
const DECOMP = {
  q: 'Complète la décomposition de 37/100 en dixièmes et centièmes.',
  options: [
    '3/10 + 7/100',
    '37/10 + 7/100',
    '3/100 + 7/10',
    '30/10 + 7/100',
  ],
  correct: 0,
  explain:
    'Les 3 lignes complètes valent 3 dixièmes et les 7 cases restantes valent 7 centièmes : 37/100 = 3/10 + 7/100.',
};

export default function Module03FractionsDecimales() {
  const navLinks = getNavLinks(3);
  const [lectures, setLectures] = useState([]);
  const [plusPick, setPlusPick] = useState(null);
  const [plusRevealed, setPlusRevealed] = useState(false);
  const [decPick, setDecPick] = useState(null);
  const [decRevealed, setDecRevealed] = useState(false);

  const s1 = lectures.length === LECTURES.length;
  const s2 = plusRevealed && plusPick === PLUS_DE_UN.correct;
  const s3 = decRevealed && decPick === DECOMP.correct;
  const allDone = s1 && s2 && s3;

  return (
    <ModuleLayout
      {...MODULE_CTX}
      moduleTitle="Les fractions décimales"
      moduleSubtitle="Écrire une quantité avec un dénominateur 10, 100 ou 1 000."
      moduleNumber={3}
      estimatedTime="12 min"
      prevLink={navLinks.prevLink}
      nextLink={allDone ? navLinks.nextLink : undefined}
      isCompleted={allDone}
    >
      <div className="max-w-3xl mx-auto space-y-6">
        <MissionBrief tag="➗ Écriture" title="Une fraction décimale raconte le découpage.">
          <p>
            Le <strong className="text-white">dénominateur</strong> dit en combien de parts égales l'unité a été
            partagée : 10, 100 ou 1 000. Le <strong className="text-white">numérateur</strong> dit combien de
            parts on prend.
          </p>
        </MissionBrief>

        {/* Étape 1 */}
        <StepCard num={1} title="Lis le dessin, écris la fraction" done={s1}>
          <div className="space-y-8">
            {LECTURES.map((item, i) =>
              i === 0 || lectures.includes(i - 1) ? (
                <div key={`${item.parts}-${item.shaded}`} className="space-y-3">
                  <div className="text-[11px] font-mono font-bold text-slate-400 uppercase tracking-wider">
                    Dessin {i + 1} / {LECTURES.length}
                  </div>
                  <UnitGrid
                    parts={item.parts}
                    shaded={item.shaded}
                    tone={item.tone}
                    showCount={false}
                    label="Quelle fraction de l'unité est coloriée ?"
                  />
                  <FractionBuilder
                    target={item.shaded}
                    den={item.parts}
                    solved={lectures.includes(i)}
                    onSolved={() => setLectures((d) => (d.includes(i) ? d : [...d, i]))}
                  />
                </div>
              ) : null
            )}
          </div>
        </StepCard>

        {/* Étape 2 */}
        <StepCard
          num={2}
          title="Et si la fraction dépasse l'unité ?"
          subtitle="Une fraction décimale peut représenter plus d'une unité entière."
          done={s2}
          locked={!s1}
        >
          <div className="space-y-4">
            <div className="text-center text-3xl text-slate-800 py-2">
              <MathText>{'$\\frac{405}{100}$'}</MathText>
            </div>

            <p className="text-sm font-semibold text-slate-700">{PLUS_DE_UN.q}</p>
            <ChoiceGrid
              options={PLUS_DE_UN.options}
              selected={plusPick}
              onSelect={setPlusPick}
              revealed={plusRevealed}
              correctIndex={PLUS_DE_UN.correct}
              cols={1}
            />
            {!plusRevealed && (
              <ValidateButton onClick={() => setPlusRevealed(true)} disabled={plusPick === null}>
                Valider
              </ValidateButton>
            )}
            {plusRevealed && (
              <>
                <Feedback tone={s2 ? 'ok' : 'ko'}>
                  {PLUS_DE_UN.explain}
                  {!s2 && (
                    <>
                      {' '}
                      <button
                        type="button"
                        onClick={() => {
                          setPlusRevealed(false);
                          setPlusPick(null);
                        }}
                        className="underline font-semibold"
                      >
                        Réessayer
                      </button>
                    </>
                  )}
                </Feedback>
                {s2 && (
                  <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="bg-white border-2 border-slate-200 rounded-2xl p-4">
                    <QuantityView value={4.05} den={100} tone="violet" />
                  </motion.div>
                )}
              </>
            )}
          </div>
        </StepCard>

        {/* Étape 3 */}
        <StepCard num={3} title="Décomposer une fraction décimale" done={s3} locked={!s2}>
          <div className="space-y-4">
            <UnitGrid parts={100} shaded={37} tone="violet" showCount={false} label="37 centièmes" />

            <p className="text-sm font-semibold text-slate-700">{DECOMP.q}</p>
            <ChoiceGrid
              options={DECOMP.options}
              selected={decPick}
              onSelect={setDecPick}
              revealed={decRevealed}
              correctIndex={DECOMP.correct}
              cols={2}
              renderOption={(o) => {
                const [a, b] = o.split(' + ');
                const f = (x) => {
                  const [n, d] = x.split('/');
                  return `\\frac{${n}}{${d}}`;
                };
                return <MathText>{`$${f(a)} + ${f(b)}$`}</MathText>;
              }}
            />
            {!decRevealed && (
              <ValidateButton onClick={() => setDecRevealed(true)} disabled={decPick === null}>
                Valider
              </ValidateButton>
            )}
            {decRevealed && (
              <Feedback tone={s3 ? 'ok' : 'ko'}>
                {DECOMP.explain}
                {!s3 && (
                  <>
                    {' '}
                    <button
                      type="button"
                      onClick={() => {
                        setDecRevealed(false);
                        setDecPick(null);
                      }}
                      className="underline font-semibold"
                    >
                      Réessayer
                    </button>
                  </>
                )}
              </Feedback>
            )}

            {s3 && (
              <Feedback tone="info">
                Retiens bien ce découpage : <strong>3 dixièmes + 7 centièmes</strong>. Au module suivant, tu vas
                voir qu'il s'écrit aussi <strong className="font-mono">0,37</strong> — et tu comprendras pourquoi.
              </Feedback>
            )}
          </div>
        </StepCard>
      </div>
    </ModuleLayout>
  );
}

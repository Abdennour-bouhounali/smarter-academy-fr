...
function CollectionAtelier({ item, solved, onSolved }) {
    ...
    <div className="space-y-5">
      ...
      {groupsOk && !revealed && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-3">
          <p className="text-sm font-semibold text-slate-700">{COMPARAISON_Q.q}</p>
          <ChoiceGrid options={COMPARAISON_Q.options} selected={pick} onSelect={setPick} revealed={revealed} correctIndex={COMPARAISON_Q.correct} cols={1} />
          <div className="text-center">
            <ValidateButton
              onClick={() => {
                setRevealed(true);
                if (pick === COMPARAISON_Q.correct) onSolved?.();
              }}
              disabled={pick === null}
            >
              Valider
      ...
      {revealed && (
        <Feedback tone={pick === COMPARAISON_Q.correct ? 'ok' : 'ko'}>
          {COMPARAISON_Q.explain}
          {pick !== COMPARAISON_Q.correct && (
            <>
              {' '}
              <button
                type="button"
  ...
  return (
    ...
    >
      <div className="max-w-3xl mx-auto space-y-6">
        <MissionBrief tag="🎈 Collection" title="Une fraction peut aussi partager un groupe d'objets.">
          <p>
        ...
        {/* Étape 1 */}
        <StepCard num={1} title="12 ballons, 3 groupes égaux" done={s1}>
        ...
        {/* Étape 2 */}
        <StepCard num={2} title="Même fraction, deux dessins" done={s2} locked={!s1}>
          <ComparaisonMemeQuantite solved={compDone} onSolved={() => setCompDone(true)} />
        </StepCard>
      </div>
    </ModuleLayout>
  );

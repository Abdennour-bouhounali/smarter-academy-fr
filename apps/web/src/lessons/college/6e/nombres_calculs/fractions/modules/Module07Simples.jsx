 ...
 * Les deux étapes restent des manipulations maison (partage à toucher, jeu
 * d'association) : ce ne sont pas des QCM, donc pas de TapQuestion ici.
 * Contrat kit respecté : onSolved inconditionnel, react() branché au même
 * point, aucun Valider superflu là où le tap suffit déjà.
 *
 * NOTE : `partName(item.num, item.den)` reprend exactement l'appel de
 * l'original — `den` est passé positionnellement en tant que `plural`
 * (toujours truthy, donc toujours la forme plurielle, qui est visuellement
 * identique au singulier pour cette liste de mots). Comportement préservé
 * à l'identique, ce n'est pas dans le périmètre de ce portage de le corriger.
 */
...
export default function Module07Simples() {
  ...
  return (
    <ContentModule
      ...
      brief={{
        tag: '🔎 Reconnaissance',
        title: 'Quatre fractions que tu croises tous les jours.',
        body: <p>Une demi-heure, un tiers de classe, un quart d'heure, un dixième de mètre : tu les connais déjà sans le savoir.</p>,
      }}
      steps={[
        {
          num: 1,
          title: 'Quatre situations du quotidien',
          done: s1,
          content: (kit) => (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {CONTEXTES.map((item, i) => (
                <ContexteCard
                  ...
                  onSolved={() => {
                    setCtxDone((d) => (d.includes(i) ? d : [...d, i]));
                    kit.react(true);
                  }}
                />
              ))}
        ...
        },
        {
          ...
          subtitle: 'Figures ↔ noms, sans indice de position.',
          done: s2,
          content: (kit) => (
            <JeuAssociation
              solved={jeuDone}

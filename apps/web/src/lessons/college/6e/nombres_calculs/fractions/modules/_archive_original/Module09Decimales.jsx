...
/**
 * Module « pont » : AUCUNE reprise du cours Nombres décimaux (dixièmes,
 * centièmes, tableau de numération, droite décimale...). On part de ce que
 * l'élève sait déjà et on construit uniquement le lien avec l'écriture
 * fractionnaire.
 */

/* ─── Étape 1 : 0,5 ↔ 1/2 ─────────────────────────────────────────── */
function BridgeUnDemi({ solved, onSolved }) {
...
/* ─── Étape 2 : 0,25 ↔ 25/100 ─────────────────────────────────────── */
function BridgeVingtCinqCentiemes({ solved, onSolved }) {
  const [revealed, setRevealed] = useState(false);
...
export default function Module09Decimales() {
  ...
  const [reglePick, setReglePick] = useState(null);
  const [regleRevealed, setRegleRevealed] = useState(false);

  const s1 = b1;
  const s2 = b2;
  const s3 = regleRevealed && reglePick === REGLE_Q.correct;
  ...
  return (
    ...
    >
      <div className="max-w-3xl mx-auto space-y-6">
        <MissionBrief tag="🌉 Pont" title="Deux notations que tu connais déjà, reliées entre elles.">
          <p>
        ...
        <StepCard num={1} title="0,5 et sa fraction" done={s1}>
          <BridgeUnDemi solved={b1} onSolved={() => setB1(true)} />
        </StepCard>

        <StepCard num={2} title="0,25 et sa fraction" done={s2} locked={!s1}>
          <BridgeVingtCinqCentiemes solved={b2} onSolved={() => setB2(true)} />
        </StepCard>

        <StepCard num={3} title="La règle, et deux rappels flash" done={s3} locked={!s2}>
          <div className="space-y-4">
            ...
            <p className="text-sm font-semibold text-slate-700">{REGLE_Q.q}</p>
            <ChoiceGrid options={REGLE_Q.options} selected={reglePick} onSelect={setReglePick} revealed={regleRevealed} correctIndex={REGLE_Q.correct} cols={1} />
            {!regleRevealed && (
              <div className="text-center">
                <ValidateButton onClick={() => setRegleRevealed(true)} disabled={reglePick === null}>
                  Valider
                </ValidateButton>
              </div>
            )}
            {regleRevealed && (
              <Feedback tone={reglePick === REGLE_Q.correct ? 'ok' : 'ko'}>
                {REGLE_Q.explain}
                {reglePick !== REGLE_Q.correct && (
                  <>
                    {' '}
                    <button
                      type="button"
                      onClick={() => {
                        setRegleRevealed(false);
                        setReglePick(null);
                      }}

...
const MODULE_COMPONENTS = {
  ...
  1: lazy(() => import('./modules/Module01Distributeur.jsx')),
  2: lazy(() => import('./modules/Module02MemeNombre.jsx')),
  3: lazy(() => import('./modules/Module03BancEssai.jsx')),
  4: lazy(() => import('./modules/Module04CompleterTableau.jsx')),
  5: lazy(() => import('./modules/Module05ChoisirStrategie.jsx')),
  6: lazy(() => import('./modules/Module06Kermesse.jsx')),
  7: lazy(() => import('./modules/Module07MissionFinale.jsx')),
};

// Chaque page de la leçon (index + modules) est enveloppée dans le provider de
// la carte des connaissances (docs/architecture/KNOWLEDGE_MAP.md).
  ...
  return (
    <Suspense fallback={<div className="min-h-screen bg-slate-50" />}>
      <LazyLessonKnowledgeProvider
        ...
        printTitle="LA PROPORTIONNALITÉ"
...
export default function proportionnaliteRoutes() {
  return [
    <Route key="proportionnalite-index" path={LESSON_BASE_PATH} element={withSuspense(LessonHome)} />,
    ...LESSON_CONFIG.modules
      .map((m) => {
        const Component = MODULE_COMPONENTS[m.number];
        if (!Component) return null;
        return <Route key={`proportionnalite-${m.id}`} path={m.path} element={withSuspense(Component)} />;
      })
      .filter(Boolean),
  ];
}

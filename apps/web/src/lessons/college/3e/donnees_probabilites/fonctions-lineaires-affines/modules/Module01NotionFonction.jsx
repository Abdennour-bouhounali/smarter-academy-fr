...
export default function Module01NotionFonction() {
  ...
  const [antecedentAnswer, setAntecedentAnswer] = useState('');
  const [isAntecedentComplete, setIsAntecedentComplete] = useState(false);
  const [antecedentError, setAntecedentError] = useState(false);
  ...
  const handleAntecedentSubmit = (e) => {
    e.preventDefault();
    const val = parseFloat(antecedentAnswer.replace(',', '.'));
    if (val === 10) {
      setIsAntecedentComplete(true);
      setAntecedentError(false);
    } else {
      setAntecedentError(true);
    }
  };
        ...
        <div className="bg-white p-6 sm:p-8 rounded-3xl shadow-sm border border-slate-200 space-y-12 overflow-hidden">
          ...
          {hasExplored && (
            <div className="space-y-6 pt-8 border-t border-slate-100 animate-in fade-in slide-in-from-bottom-4">
              <ConceptCard label="Définition" emoji="💡" color="indigo">
                <p>Tu viens de découvrir une règle mathématique cachée ! Quand la distance change, le prix change toujours selon la même logique. En mathématiques, on appelle cette règle une <strong>fonction</strong>.</p>
              </ConceptCard>
          ...
          {isTrainingComplete && (
            <div className="space-y-6 pt-8 border-t border-slate-100 animate-in fade-in slide-in-from-bottom-4">
              ...
              {isErrorDiagnosed && (
                <ConceptCard label="Piège à éviter" emoji="⚠️" color="amber">
                  <div>
            ...
            <div className="space-y-6 pt-8 border-t border-slate-100 animate-in fade-in slide-in-from-bottom-4">
              ...
              <div className="bg-indigo-50 p-6 sm:p-8 rounded-3xl border-2 border-indigo-100">
                ...
                <div className="flex flex-col items-center">
                  ...
                  {antecedentError && !isAntecedentComplete && (
                    <p className="text-red-500 text-center font-bold -mt-4 mb-4">Vérifie ton calcul : 1,5 × distance + 2 = 17.</p>
                  )}
                </div>

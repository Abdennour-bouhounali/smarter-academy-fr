...
// le `number` du module, pas par le slug. Leçon construite d'emblée sur le
// lesson kit (common/kit) — pas de version pré-kit, pas d'archive.
const MODULE_COMPONENTS = {
  0: lazy(() => import('./modules/Module00Diagnostic.jsx')),
  1: lazy(() => import('./modules/Module01MachineMysterieuse.jsx')),
  2: lazy(() => import('./modules/Module02ImageEtAntecedent.jsx')),
  3: lazy(() => import('./modules/Module03TableauDeValeurs.jsx')),
  4: lazy(() => import('./modules/Module04DuTableauAuRepere.jsx')),
  5: lazy(() => import('./modules/Module05LineaireAffine.jsx')),
  6: lazy(() => import('./modules/Module06RetrouverLaRegle.jsx')),
  7: lazy(() => import('./modules/Module07DansLaVraieVie.jsx')),
...
export default function fonctions3eRoutes() {
  return [
    <Route key="fonctions-3e-index" path={LESSON_BASE_PATH} element={withSuspense(LessonHome)} />,
    ...LESSON_CONFIG.modules
      .map((m) => {
        const Component = MODULE_COMPONENTS[m.number];
        if (!Component) return null;
        return <Route key={`fonctions-3e-${m.id}`} path={m.path} element={withSuspense(Component)} />;
      })
      .filter(Boolean),
  ];
}

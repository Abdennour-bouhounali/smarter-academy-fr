...
import { compareMathExpressions, isMissingSquareRoot, computePythagoreanLeg } from '@smarter-academy/core';
...
export default function Module03CalculCote() {
  ...
  const unknownSide = computePythagoreanLeg(hypotenuse, knownSide); // 12
  ...
  const validateAnswer = (val) => {
    ...
    return { isCorrect: isEquivalent || isApprox };
  };
  ...
  const {
    status: answerStatus,
    specificFeedback: adaptiveFeedback,
    currentGuidance,
    submitAnswer,
    requestHint,
    hasMoreHints,
  ...
  const isSolutionRevealed = answerStatus === 'solution_viewed';

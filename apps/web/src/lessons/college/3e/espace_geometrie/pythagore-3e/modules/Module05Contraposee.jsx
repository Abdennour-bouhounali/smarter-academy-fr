...
export default function Module05Contraposee() {
  ...
  const [sqHypoAnswer, setSqHypoAnswer] = useState('');
  const [sqSumAnswer, setSqSumAnswer] = useState('');

  const adaptiveSqHypo = useAdaptiveExercise({
    validate: (val) => {
      const isEquivalent = compareMathExpressions(val, (c * c).toString());
      const num = parseFloat(val.replace(',', '.'));
      return { isCorrect: isEquivalent || (!isNaN(num) && num === c * c) };
    },
    guidanceSteps: [
  ...
  const isSqHypoSolutionRevealed = adaptiveSqHypo.status === 'solution_viewed';

  const adaptiveSqSum = useAdaptiveExercise({
    validate: (val) => {
      const isEquivalent = compareMathExpressions(val, (a * a + b * b).toString());
      const num = parseFloat(val.replace(',', '.'));
      return { isCorrect: isEquivalent || (!isNaN(num) && num === a * a + b * b) };
    },
    guidanceSteps: [
  ...
  const isSqSumSolutionRevealed = adaptiveSqSum.status === 'solution_viewed';
  ...
  const checkSqHypo = () => {
    adaptiveSqHypo.submitAnswer(sqHypoAnswer);
  };

  const checkSqSum = () => {
    adaptiveSqSum.submitAnswer(sqSumAnswer);
  };
                  ...
                  {step === 2 ? (
                    <div>
                      <div className="flex gap-2 items-center">
                        ...
                        <MathInput
                          value={sqHypoAnswer}
                          onChange={(val) => { setSqHypoAnswer(val); }}
                          onCommit={checkSqHypo}
                          className="w-20"
                          disabled={isSqHypoSolutionRevealed}
                        />
                        {!isSqHypoSolutionRevealed && (
                      ...
                      <AdaptiveFeedback
                        ...
                        feedback={adaptiveSqHypo.specificFeedback}
                        ...
                        canViewSolution={adaptiveSqHypo.canViewSolution}
                        onViewSolution={adaptiveSqHypo.viewSolution}
                      />
                      {isSqHypoSolutionRevealed && (
                        <div className="mt-4">
                  ...
                  {step === 3 ? (
                    <div>
                      <div className="flex gap-2 flex-wrap items-center">
                        ...
                        <MathInput
                          value={sqSumAnswer}
                          onChange={(val) => { setSqSumAnswer(val); }}
                          onCommit={checkSqSum}
                          className="w-20"
                          disabled={isSqSumSolutionRevealed}
                        />
                        {!isSqSumSolutionRevealed && (
                      ...
                      <AdaptiveFeedback
                        ...
                        feedback={adaptiveSqSum.specificFeedback}
                        ...
                        canViewSolution={adaptiveSqSum.canViewSolution}
                        onViewSolution={adaptiveSqSum.viewSolution}
                      />
                      {isSqSumSolutionRevealed && (
                        <div className="mt-4">

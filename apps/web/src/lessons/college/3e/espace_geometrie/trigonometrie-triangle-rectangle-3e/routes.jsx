...
// connaissances : il cumule les apports des modules validés et accueille les
// connaissances posées par les <KnowledgeBrick> au fil des étapes.
...
export default function trigo3eRoutes() {
  return [
    <Route key="trigonometrie-triangle-rectangle-3e-index" path={LESSON_BASE_PATH} element={withSuspense(LessonHome)} />,
    ...LESSON_CONFIG.modules
      .map((m) => {
        const Component = MODULE_COMPONENTS[m.number];
        if (!Component) return null;
        return <Route key={`trigonometrie-triangle-rectangle-3e-${m.id}`} path={m.path} element={withSuspense(Component)} />;
      })
      .filter(Boolean),
  ];
}

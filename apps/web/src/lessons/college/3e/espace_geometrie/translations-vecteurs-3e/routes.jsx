...
// le `number` du module, pas par le slug.
const MODULE_COMPONENTS = {
  0: lazy(() => import('./modules/Module00Diagnostic.jsx')),
  1: lazy(() => import('./modules/Module01LeMemeTrajet.jsx')),
  2: lazy(() => import('./modules/Module02DirectionSensLongueur.jsx')),
  3: lazy(() => import('./modules/Module03ToutBouge.jsx')),
  4: lazy(() => import('./modules/Module04FlecheVagabonde.jsx')),
  5: lazy(() => import('./modules/Module05DeuxNombres.jsx')),
  6: lazy(() => import('./modules/Module06VecteurEtTranslation.jsx')),
  ...
  return (
    <Suspense fallback={<div className="min-h-screen bg-slate-50" />}>
      <LazyLessonKnowledgeProvider
        ...
        printTitle="TRANSLATIONS ET VECTEURS"
...
export default function translationsVecteurs3eRoutes() {
  return [
    <Route key="translations-vecteurs-3e-index" path={LESSON_BASE_PATH} element={withSuspense(LessonHome)} />,
    ...LESSON_CONFIG.modules
      .map((m) => {
        const Component = MODULE_COMPONENTS[m.number];
        if (!Component) return null;
        return <Route key={`translations-vecteurs-3e-${m.id}`} path={m.path} element={withSuspense(Component)} />;
      })
      .filter(Boolean),
  ];
}

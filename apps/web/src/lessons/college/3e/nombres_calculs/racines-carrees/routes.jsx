...
const MODULE_COMPONENTS = {
  ...
  1: lazy(() => import('./modules/Module01JardinCarre.jsx')),
  2: lazy(() => import('./modules/Module02CarresParfaits.jsx')),
  3: lazy(() => import('./modules/Module03CarreAReconstruire.jsx')),
  4: lazy(() => import('./modules/Module04PavageEtProduit.jsx')),
  5: lazy(() => import('./modules/Module05SimplifierUneRacine.jsx')),
  6: lazy(() => import('./modules/Module06PythagoreEtCie.jsx')),
...
export default function racinesCarrees3eRoutes() {
  return [
    <Route key="racines-carrees-3e-index" path={LESSON_BASE_PATH} element={withSuspense(LessonHome)} />,
    ...LESSON_CONFIG.modules
      .map((m) => {
        const Component = MODULE_COMPONENTS[m.number];
        if (!Component) return null;
        return <Route key={`racines-carrees-3e-${m.id}`} path={m.path} element={withSuspense(Component)} />;
      })
      .filter(Boolean),
  ];
}

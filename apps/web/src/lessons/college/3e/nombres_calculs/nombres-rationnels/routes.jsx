...
const MODULE_COMPONENTS = {
  ...
  1: lazy(() => import('./modules/Module01DeuxNomsUnNombre.jsx')),
  2: lazy(() => import('./modules/Module02RendreIrreductible.jsx')),
  3: lazy(() => import('./modules/Module03Comparer.jsx')),
  4: lazy(() => import('./modules/Module04MemeDecoupe.jsx')),
  5: lazy(() => import('./modules/Module05FractionDeFraction.jsx')),
  6: lazy(() => import('./modules/Module06DansQuelOrdre.jsx')),
  7: lazy(() => import('./modules/Module07BudgetDuClub.jsx')),
  ...
  return (
    <Suspense fallback={<div className="min-h-screen bg-slate-50" />}>
      <LazyLessonKnowledgeProvider
        ...
        printTitle="LES NOMBRES RATIONNELS"
...
export default function nombresRationnelsRoutes() {
  return [
    <Route key="nombres-rationnels-index" path={LESSON_BASE_PATH} element={withSuspense(LessonHome)} />,
    ...LESSON_CONFIG.modules
      .map((m) => {
        const Component = MODULE_COMPONENTS[m.number];
        if (!Component) return null;
        return <Route key={`nombres-rationnels-${m.id}`} path={m.path} element={withSuspense(Component)} />;
      })
      .filter(Boolean),
  ];
}

  ...
  return (
    <Suspense fallback={<div className="min-h-screen bg-slate-50" />}>
      <LessonKnowledgeProvider
        ...
        printTitle="L’ESPACE : VECTEURS ET COORDONNÉES"
...
export default function espaceVecteursCoordonnees1ereRoutes() {
  return [
    <Route key="espace-vec-1ere-index" path={LESSON_BASE_PATH} element={withSuspense(LessonHome)} />,
    ...LESSON_CONFIG.modules
      .map((m) => {
        const Component = MODULE_COMPONENTS[m.number];
        if (!Component) return null;
        return <Route key={`espace-vec-1ere-${m.id}`} path={m.path} element={withSuspense(Component)} />;
      })
      .filter(Boolean),
  ];
}

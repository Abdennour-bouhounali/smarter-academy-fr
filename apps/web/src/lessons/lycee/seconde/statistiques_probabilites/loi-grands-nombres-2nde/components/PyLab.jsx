 ...
 * code fait. Le hasard est injecté par graine : NOUVELLE à chaque exécution
 * par défaut (une simulation doit fluctuer), fixable pour un test.
 *
...
export default function PyLab({
  ...
  readOnly = false,
  showEnv = true,
  seed = null,           // null (défaut) : chaque exécution tire une nouvelle graine
                         //   — indispensable dès qu'un programme utilise randint,
                         //   sinon le hasard donnerait toujours le même résultat.
                         // Un nombre : exécution reproductible (tests, captures).
  height = 'h-44',
  maxSteps = 20000,      // une simulation de 10 000 tours consomme ~40 000 pas
  ...
  const run = () => {
    const graine = seed === null ? (Date.now() ^ Math.floor(Math.random() * 0xffffffff)) >>> 0 : seed;
    const r = pyRun(source, { rng: makeRng(graine), maxSteps });
    setResult(r);
    onRun?.({ ...r, source });
  };

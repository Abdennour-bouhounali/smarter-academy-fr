 ...
 * Rien n'est appelé croissante / tableau de variations avant le pied de module.
 */
...
export default function Module01LeRandonneur() {
  ...
  const steps = [
    ...
    },
    {
      ...
      content: (kit) => (
        <div className="space-y-3">
          <TrailLab {...PROBE} value={x} visited={visited} onChange={(v) => move(v, kit.react)} disabled={done2 || !done1} />
          {done2 ? (
            <Feedback tone="ok">
  ...
  return (
    <ContentModule ctx={MODULE_CTX} navLinks={getNavLinks(1)} moduleNumber={1}
      ...
      steps={steps}
      footer={<KnowledgeSnapshot moduleNumber={1}><strong>Les mots justes.</strong> « Ça monte » : on dit que h est <strong>croissante</strong> sur [0 ; 3] ; « ça descend » : <strong>décroissante</strong> sur [3 ; 6]. Et la piste peinte a un nom — module 3, le tableau de variations. Avant : ce que ces mots veulent dire exactement.</KnowledgeSnapshot>} />
  );
}

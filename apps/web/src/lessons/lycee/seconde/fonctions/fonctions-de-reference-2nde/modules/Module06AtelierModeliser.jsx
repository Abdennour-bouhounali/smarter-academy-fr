...
export default function Module06AtelierModeliser() {
  ...
  const steps = [
    ...
    },
    {
      ...
      content: (
        <BatchChoiceQuestion intro={<p className="text-sm text-slate-700"><MathText>{'$t(v) = \\dfrac{120}{v}$'}</MathText> — une fonction inverse, à un facteur près.</p>}
          rows={[
    ...
    },
    {
      ...
      content: (
        <BatchChoiceQuestion intro={<p className="text-sm text-slate-700">La fonction valeur absolue, telle quelle.</p>}
          rows={[
      ...
      content: (kit) => (
        <div className="space-y-3">
          <ValueTable columns={[{ id: 'sq', label: <MathText>{'$x^2$'}</MathText>, fn: (x) => x * x }, { id: 'inv', label: <MathText>{'$1/x$'}</MathText>, fn: (x) => 1 / x }, { id: 'abs', label: <MathText>{'$|x|$'}</MathText>, fn: (x) => Math.abs(x) }]}
            xs={[-2, -1, -0.5, 0.5, 1, 2]} tested={tested} compare={false} disabled={q4}

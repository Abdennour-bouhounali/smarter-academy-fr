 ...
 *   Les trois connaissances du module ne vivaient que dans les `Feedback` de
 *   fin d'étape et dans l'« À retenir » du pied. L'ordre est maintenant
 *   geste → brique → demande :
 *     étape 1  x² = 4 puis x² = −1
 *     étape 2  1/x = 2
 *     étape 3  |x| = 3  → brique `methode-antecedents-reference` (les trois
 *                         comptages, une fois les trois sondes passées)
 *     étape 4  la position relative, `requires` sur les trois portraits, puis
 *              les briques `regle-ordre-references` et `formule-references` —
 *              APRÈS la question, parce que la ranger avant en donnerait la
 *              réponse.
 *
 * MANIPULATION JAMAIS GELÉE. Les trois sondes horizontales restaient
 * `disabled` dès l'étape réussie : impossible de revenir compter les points
 * communs avec y = 0 ou y = −1. Elles restent vivantes ; seuls les verrous
 * d'ANTÉRIORITÉ (`!done1`, `!seen2`) demeurent.
 */
  ...
  const steps = [
    {
      ...
      content: (kit) => (
        <div className="space-y-3">
          <FunctionProbe f={SQUARE} range={SQUARE_RANGE} unit={34} unitY={30} mode="y" value={y1} yStep={1}
    ...
    },
    {
      ...
      content: (kit) => (
        <div className="space-y-3">
          <FunctionProbe f={INVERSE} range={INVERSE_RANGE} unit={34} mode="y" value={y2} yStep={0.5} disabled={!done1}
    ...
    },
    {
      ...
      content: (kit) => (
        <div className="space-y-3">
          <FunctionProbe f={ABS} range={ABS_RANGE} unit={34} unitY={34} mode="y" value={y3} yStep={1} disabled={!seen2}
          ...
          {seen3 ? (
            <>
              ...
              <KnowledgeBrick
                id="methode-antecedents-reference"
                variant="new"
                lead="Tes trois sondes horizontales ont compté, à chaque fois, les points communs entre la courbe et la droite y = k. Voilà les trois comptages."
              />
            </>
          ) : (
    ...
    },
    {
      ...
      content: (
        <div className="space-y-3">
          <BatchChoiceQuestion
            requires={['fonction-carre', 'fonction-inverse', 'fonction-valeur-absolue', 'methode-antecedents-reference']}
            intro={(
          ...
          {q4 && (
            <>
              <KnowledgeBrick
                id="regle-ordre-references"
                variant="new"
                lead="L’ordre que tu viens de lire sur les trois courbes, écrit une fois — avec le point (1 ; 1) où elles se rejoignent."
              />
              <KnowledgeBrick
                id="formule-references"
                variant="new"
                lead="Et les trois expressions elles-mêmes, à ne plus confondre."
              />
            </>
          )}
        </div>

...
const KEYWORDS = new Set(['if', 'elif', 'else', 'for', 'while', 'in', 'range', 'True', 'False', 'and', 'or', 'not', 'def', 'return']);
...
function parseExpr(tokens, env, line, rng, callUser = null) {
  let pos = 0;
  ...
  function primary() {
    ...
    if (tk.v === '[') {
      pos += 1;
      const items = [];
      if (!(peek() && peek().v === ']')) { do { items.push(orExpr()); } while (eat(',')); }
      if (!eat(']')) throw new PyError('crochet non fermé', line);
      return items;
    }
    ...
    if (tk.t === 'name') {
      ...
      if (peek() && peek().v === '(') {
        ...
        if (callUser && callUser.has(tk.v)) return callUser.call(tk.v, args, line);
      ...
      while (peek() && peek().v === '[') {
        ...
        const idx = orExpr();
        if (!eat(']')) throw new PyError('crochet non fermé', line);
        if (!Array.isArray(base)) throw new PyError(`« ${tk.v} » n'est pas une liste`, line);
        const k = idx < 0 ? base.length + idx : idx;
        if (k < 0 || k >= base.length) throw new PyError(`indice ${idx} hors de la liste (taille ${base.length})`, line);
        base = base[k];
      }
      return base;
    }
...
function callBuiltin(name, args, line, rng) {
  switch (name) {
    ...
    case 'sum': {
      if (!Array.isArray(args[0])) throw new PyError('sum(...) attend une liste', line);
      return args[0].reduce((a, b) => a + b, 0);
    }
    case 'min': {
      const xs = Array.isArray(args[0]) ? args[0] : args;
      if (!xs.length) throw new PyError('min(...) sur une liste vide', line);
      return Math.min(...xs);
    }
    case 'max': {
      const xs = Array.isArray(args[0]) ? args[0] : args;
      if (!xs.length) throw new PyError('max(...) sur une liste vide', line);
      return Math.max(...xs);
    }
...
function parseBlock(lines, start, indent) {
  ...
  while (i < lines.length) {
    ...
    if (text.startsWith('def ')) {
      ...
      if (!m) throw new PyError('une définition s’écrit : def nom(parametres) :', no);
      const params = m[2].trim() === '' ? [] : m[2].split(',').map((a) => a.trim());
      const [inner, next] = parseBlock(lines, i + 1, indent + 1);
      if (!inner.length) throw new PyError(`le corps de la fonction « ${m[1]} » est vide`, no);
      body.push({ kind: 'def', name: m[1], params, body: inner, no });
      i = next; continue;
    }
...
export function pyRun(source, { rng = Math.random, maxSteps = 20000 } = {}) {
  ...
  // Pile de portées : le dernier cadre est la portée courante. Une fonction ne
  // voit que ses paramètres et ses variables locales (plus les fonctions
  // globales) — c'est ce qui rend la leçon honnête sur la notion de portée.
  const scopes = [env];
  ...
  const callUser = {
    has: (name) => funcs.has(name),
    call: (name, args, no) => {
      const fn = funcs.get(name);
      if (args.length !== fn.params.length) {
        throw new PyError(`« ${name} » attend ${fn.params.length} argument(s), il en reçoit ${args.length}`, no);
      }
      if (scopes.length > 60) throw new PyError('trop d’appels imbriqués (récursion infinie ?)', no);
      const local = {};
      fn.params.forEach((pn, k) => { local[pn] = args[k]; });
      scopes.push(local);
      try {
        const r = run(fn.body);
  ...
  const evalExpr = (src, no) => parseExpr(tokenize(src, no), scope(), no, rng, callUser);
    ...
    for (const node of block) {
      ...
      if (node.kind === 'stmt') {
        ...
        if (t === 'return' || t.startsWith('return ')) {
          ...
          return { type: RETURN, value: expr === '' ? null : evalExpr(expr, node.no) };
        }
        const ap = t.match(/^([A-Za-z_]\w*)\.append\((.+)\)$/);
        if (ap) {
          const target = scope()[ap[1]];
          if (!Array.isArray(target)) throw new PyError(`« ${ap[1]} » n'est pas une liste`, node.no);
          target.push(evalExpr(ap[2], node.no));
          continue;
        }
        ...
        if (op === '=') sc[name] = value;
          ...
          if (!(name in sc)) throw new PyError(`la variable « ${name} » n'existe pas encore`, node.no);
          sc[name] = op === '+=' ? sc[name] + value : op === '-=' ? sc[name] - value : sc[name] * value;
        }
        continue;
      }

      if (node.kind === 'def') { funcs.set(node.name, node); continue; }

      if (node.kind === 'if' || node.kind === 'elif') {
        if (node.kind === 'elif' && skipElse) continue;
        const ok = truthy(evalExpr(node.cond, node.no));
        if (ok) { const r = run(node.body); if (r) return r; skipElse = true; } else if (node.kind === 'if') skipElse = false;
        continue;
      }
      if (node.kind === 'else') { if (!skipElse) { const r = run(node.body); if (r) return r; } skipElse = false; continue; }
      ...
      if (node.kind === 'for') {
        ...
        for (let v = from; step > 0 ? v < to : v > to; v += step) {
          scope()[node.varName] = v;
          steps += 1;
          if (steps > maxSteps) throw new PyError('le programme ne s’arrête pas (boucle infinie ?)', node.no);
          const r = run(node.body);
          if (r) return r;
        }
        continue;
      }

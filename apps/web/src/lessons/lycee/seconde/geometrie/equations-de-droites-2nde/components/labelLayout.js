/**
 * (copié de vecteurs-2nde le 2026-09-05, adapté ici — INTERACTION_PEDAGOGY §6ter.6 : on copie, on n’importe pas entre leçons)
...
function arrowCandidates(from, to, w, h) {
  ...
  const out = [];
  for (const d of [11, 18, 26]) {
    for (const t of [0.5, 0.3, 0.7, 0.15, 0.85]) {
      for (const side of [1, -1]) {
        const cx = from.x + dx * t + n.x * side * (d + Math.min(w, h) / 2);
        const cy = from.y + dy * t + n.y * side * (d + Math.min(w, h) / 2);
        out.push({ x: cx - w / 2, y: cy - h / 2, w, h });
      }
    }
  }
...
/**
 * @param labels    [{ id, text, size, kind: 'point'|'arrow', anchor:{x,y} | from,to, prefer? , pad? }]
...
export function placeLabels(labels, obstacles, frame) {
  ...
  for (const l of labels) {
    ...
    const cands = l.kind === 'arrow'
      ? arrowCandidates(l.from, l.to, w, h)

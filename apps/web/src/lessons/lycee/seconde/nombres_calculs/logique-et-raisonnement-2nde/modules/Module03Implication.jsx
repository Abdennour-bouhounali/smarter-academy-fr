      ...
      steps={[
        {
          ...
          content: (kit) => (
            <div className="space-y-3">
              ...
              {/* Les deux sens viennent d'être testés sur les quatre cases :
                  l'implication et sa réciproque prennent sens ici, avant la
                  question qui les nomme. */}
              {seen.size >= 2 && (
                <>
                  <KnowledgeBrick
                    id="implication"
                    variant="new"
                    compact
                    lead={<>Le sens direct que tu viens de tester, « multiple de 4 ⇒ pair », interdit une seule case.</>}
                  />
                  <KnowledgeBrick
                    id="reciproque"
                    variant="new"
                    compact
                    lead={<>Et le sens que tu viens d’essayer en second, « pair ⇒ multiple de 4 », est une AUTRE affirmation.</>}
                  />
                </>
              )}
              {seen.size >= 2 && (
                <TapQuestion prompt="Que montre le test des deux sens ?" options={['« Multiple de 4 ⇒ pair » est vraie, mais sa réciproque est fausse (2, 6, 10… sont pairs sans être multiples de 4)', 'Les deux sens sont vrais', 'Les deux sens sont faux']} cols={1} correct={0}
                  requires={['implication', 'reciproque']}
            ...
            <div className="space-y-3">
              ...
              {seen2.size >= 2 && (
                <TapQuestion prompt="Sur ce domaine (3 à 20), que peut-on dire ?" options={['« Premier ⇒ impair » tient, mais la réciproque tombe sur 9 (impair, pas premier)', 'Les deux sens tiennent', 'Aucun des deux ne tient']} cols={1} correct={0}
                  requires={['implication', 'reciproque', 'nombre-premier']}
        ...
        },
        {
          ...
          content: (
            <div className="space-y-3">
              {/* La distinction que la série va exiger — contraposée contre
                  réciproque — doit être posée avant, pas devinée pendant. */}
              <KnowledgeBrick
                id="contraposee"
                variant="new"
                lead={<>Une troisième phrase, à ne pas confondre avec la réciproque que tu viens de tester.</>}
              />
              <BatchChoiceQuestion intro={<p className="text-sm text-slate-600">On part de : « Si n est multiple de 4, alors n est pair. »</p>} rows={[
                ...
                requires={['implication', 'reciproque', 'contraposee']}
              ...
              {/* La série vient de fixer le seul cas interdit dans les deux
                  exemples : c'est le réflexe à garder. */}
              {contraDone && (
                <KnowledgeBrick
                  id="mem-cas-interdit"
                  variant="new"
                  lead={<>Ce que « ⇒ » a dit, et rien de plus, dans les deux exemples du module.</>}
                />
              )}
            </div>
          ),

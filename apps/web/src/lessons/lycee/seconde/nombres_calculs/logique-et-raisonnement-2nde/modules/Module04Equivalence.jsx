    ...
    <ContentModule
      ...
      steps={[
            ...
            <div className="space-y-3">
              ...
              {seen.size >= 2 && (
                <TapQuestion prompt="Pourquoi « x² = 9 ⇔ x = 3 » est-elle fausse ?" options={['Parce que le sens « x² = 9 ⇒ x = 3 » tombe sur −3 : (−3)² = 9 mais −3 ≠ 3', 'Parce que 9 n’est pas un carré', 'Parce que les deux sens sont faux']} cols={1} correct={0}
                  requires={['implication', 'reciproque', 'carre-nombre']}
              ...
              {/* La question qui vient d'être validée donne exactement le
                  contenu de la règle : x² = 9 oublie le signe. */}
              {breakDone && (
                <KnowledgeBrick
                  id="regle-equivalence-carre"
                  variant="new"
                  compact
                  lead={<>C’est le piège que tu viens de trouver, formulé une bonne fois.</>}
                />
              )}
            </div>
          ),
        },
        {
          ...
          content: (kit) => (
            <div className="space-y-3">
              ...
              {/* Les deux sens viennent de tenir, cases interdites vides des
                  deux côtés : c'est l'instant où « équivalence » a un sens
                  positif, avant que la question ne l'exige. */}
              {seen2.size >= 2 && (
                <KnowledgeBrick
                  id="equivalence"
                  variant="new"
                  lead={<>Cette fois les DEUX sens tiennent — regarde les deux cases interdites, vides toutes les deux.</>}
                />
              )}
              {seen2.size >= 2 && (
                <TapQuestion prompt="Maintenant que peut-on écrire ?" options={['x² = 9 ⇔ (x = 3 ou x = −3) : les deux cases interdites sont vides', 'x² = 9 ⇒ x = 3 seulement', 'Rien de plus qu’avant']} cols={1} correct={0}
                  requires={['equivalence', 'regle-equivalence-carre']}
        ...
        },
        {
          ...
          content: (
            <div className="space-y-3">
              {/* La série va demander de choisir entre ⇒ et ⇔ sur quatre
                  couples : la méthode se pose avant, pas pendant. */}
              <KnowledgeBrick
                id="methode-choisir-symbole"
                variant="new"
                lead={<>Tu viens de le faire deux fois à la main (module 3 et cette étape) : voici la méthode qui généralise.</>}
              />
              <BatchChoiceQuestion intro={<p className="text-sm text-slate-600">Pour chaque couple, quel symbole convient ?</p>} rows={[
                ...
                requires={['methode-choisir-symbole', 'equivalence', 'implication']}

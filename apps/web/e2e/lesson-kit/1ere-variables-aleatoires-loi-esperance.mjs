/**
 * 1ère spé — « Variables aléatoires : loi et espérance ».
 *
 * Vite détaché depuis apps/web/ :
 *   cd apps/web && (setsid nohup npx vite --port 5284 --strictPort > e2e/lesson-kit/shots/vite-5284.log 2>&1 </dev/null &)
 *   node apps/web/e2e/lesson-kit/1ere-variables-aleatoires-loi-esperance.mjs
 *
...
const BASE = process.env.KIT_BASE || 'http://localhost:5284';
const LESSON = '/courses/lycee/premiere_specialite/probabilites/variables-aleatoires-loi-esperance-1ere';
const KEY = 'u_anon_smarter_lesson_variables-aleatoires-loi-esperance-1ere';

const M = {
  0: `${BASE}/courses/lycee/premiere_specialite/probabilites/variables-aleatoires-loi-esperance-1ere/mission-de-depart`,
  1: `${BASE}/courses/lycee/premiere_specialite/probabilites/variables-aleatoires-loi-esperance-1ere/la-roue-et-le-grand-livre`,
  2: `${BASE}/courses/lycee/premiere_specialite/probabilites/variables-aleatoires-loi-esperance-1ere/un-nombre-par-issue`,
  3: `${BASE}/courses/lycee/premiere_specialite/probabilites/variables-aleatoires-loi-esperance-1ere/la-loi-et-son-tableau`,
  4: `${BASE}/courses/lycee/premiere_specialite/probabilites/variables-aleatoires-loi-esperance-1ere/calculer-l-esperance`,
  5: `${BASE}/courses/lycee/premiere_specialite/probabilites/variables-aleatoires-loi-esperance-1ere/ce-que-l-esperance-veut-dire`,
  6: `${BASE}/courses/lycee/premiere_specialite/probabilites/variables-aleatoires-loi-esperance-1ere/decider-avec-l-esperance`,
  7: `${BASE}/courses/lycee/premiere_specialite/probabilites/variables-aleatoires-loi-esperance-1ere/mission-finale-la-roue`,
};

/** Apports de chaque module à la carte — miroir de knowledge.jsx. */
const CONTRIB = {
  1: ['hasard-moyenne-previsible', 'moyenne-hors-des-valeurs'],
  2: ['variable-aleatoire', 'valeurs-prises'],
  3: ['loi-de-probabilite', 'somme-des-probabilites-vaut-1', 'tableau-de-loi'],
  4: ['esperance', 'methode-calculer-esperance', 'mem-esperance'],
  5: ['esperance-moyenne-long-terme', 'benefice-espere'],
  6: ['jeu-equitable', 'decider-par-esperance', 'mem-decider'],
};
...
// ── Index ────────────────────────────────────────────────────────────────
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}variables-aleatoires-loi-esperance-1ere-index.png`, fullPage: true });
  await ctx.close();
}
...
// ── Modules d'enseignement : manipulables, non gelés, mise en page saine ──
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}variables-aleatoires-loi-esperance-1ere-m1.png`, fullPage: true });
  await ctx.close();
}
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}variables-aleatoires-loi-esperance-1ere-m2.png`, fullPage: true });
  await ctx.close();
}
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}variables-aleatoires-loi-esperance-1ere-m3.png`, fullPage: true });
  await ctx.close();
}
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}variables-aleatoires-loi-esperance-1ere-m4.png`, fullPage: true });
  await ctx.close();
}
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}variables-aleatoires-loi-esperance-1ere-m5.png`, fullPage: true });
  await ctx.close();
}
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}variables-aleatoires-loi-esperance-1ere-m6.png`, fullPage: true });
  await ctx.close();
}
...
// ── Le boss ──────────────────────────────────────────────────────────────
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}variables-aleatoires-loi-esperance-1ere-boss.png`, fullPage: true });
  await ctx.close();
}
...
for (const n of [1, 7]) {
  ...
  if (n === 1) await page.screenshot({ path: `${SHOT_DIR}variables-aleatoires-loi-esperance-1ere-m1-mobile.png`, fullPage: true });
  await ctx.close();
}

// Carte des connaissances — Nombres réels 2nde : progression cumulative,
// « À retenir » = snapshot de la carte, tiroir = même état, impression, responsive.
// Run: node apps/web/e2e/lesson-kit/2nde-nombres-reels-carte.mjs   (vite on :5240, started from apps/web/)
import {
...
const SLUG = {
  1: 'le-zoom-infini',
  2: 'les-familles-de-nombres',
  3: 'decimal-ou-pas',
  4: 'exact-ou-approche',
  5: 'encadrer-et-comparer',
  boss: 'mission-finale-la-diagonale',
};
...
const CONTRIB = {
  1: ['droite-reelle', 'trois-comportements', 'methode-encadrer-decimales'],
  2: ['familles-emboitees', 'decimal', 'rationnel', 'irrationnel'],
  3: ['regle-restes-division', 'regle-fraction-finie-ou-periodique', 'exact-approche', 'mem-exact-approche'],
  4: ['arrondi-troncature', 'regle-aucun-decimal-nest-racine', 'mem-exact-puis-arrondir'],
  5: ['methode-encadrer-racine', 'methode-comparer-reels', 'regle-sens-arrondi'],
};
...
/* ── Départ : carte vide sur l'index ─────────────────────────────────────── */
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}nr-carte-index-empty.png` });
  await ctx.close();
}
...
for (const n of [1, 2, 3, 4, 5]) {
  ...
  if (n === 2) {
    ...
    await page.locator('#km-root [data-km-item="familles-emboitees"]').click(); await settle(page, 300);
    check('M2: navigation → item detail opens', /Retour à la carte/.test(await body(page)));
    await page.locator('#km-root button:has-text("Retour à la carte")').click(); await settle(page, 300);
    check('M2: back to the category grid', (await page.locator('#km-root [data-km-navview] [data-km-item]').count()) === exp.length);
  }
  if (n === 4) {
    ...
    check('M4: print header carries the lesson title, not « LES VECTEURS »', /LES NOMBRES RÉELS/.test(printText) && !/LES VECTEURS/.test(printText));
    check('M4: print view shows nothing from M5', !/Encadrer une racine/.test(printText));
    check('M4: print CSS swaps screen → print view', screenHidden && printShown);
    await page.screenshot({ path: `${SHOT_DIR}nr-carte-m4-print.png`, fullPage: true });
  ...
  if (n === 5) {
    ...
    await page.screenshot({ path: `${SHOT_DIR}nr-carte-m5-final.png`, fullPage: true });
  }
  await ctx.close();
}
...
/* ── Boss : la synthèse EST la carte complète ────────────────────────────── */
{
  ...
  check('boss synthèse: old hand-written synthèse gone', !/Synthèse : la diagonale du carreau/.test(await body(page)));
  ...
  await page.screenshot({ path: `${SHOT_DIR}nr-carte-boss.png`, fullPage: true });
  await ctx.close();
}
...
/* ── Mobile ──────────────────────────────────────────────────────────────── */
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}nr-carte-mobile.png` });
  await ctx.close();
}

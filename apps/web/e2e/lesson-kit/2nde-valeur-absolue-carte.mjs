// Carte des connaissances — Valeur absolue et distance 2nde : progression cumulative,
// « À retenir » = snapshot de la carte, tiroir = même état, impression, responsive.
// Run: node apps/web/e2e/lesson-kit/2nde-valeur-absolue-carte.mjs   (vite on :5240, started from apps/web/)
import {
...
const SLUG = {
  1: 'deux-bateaux-une-distance',
  2: 'calculer-une-valeur-absolue',
  3: 'la-distance-entre-deux-nombres',
  4: 'le-faisceau',
  5: 'situations',
  boss: 'mission-finale-le-phare',
};
...
const CONTRIB = {
  1: ['valeur-absolue-distance-zero', 'regle-opposes-meme-distance', 'mem-valeur-absolue-positive'],
  2: ['regle-calcul-valeur-absolue', 'methode-calculer-expression-absolue'],
  3: ['distance-deux-nombres', 'regle-distance-invariante', 'mem-distance'],
  4: ['faisceau-intervalle', 'equation-distance-egale', 'methode-centre-rayon', 'mem-faisceau'],
  5: ['methode-tolerance', 'regle-deux-positions'],
};
...
/* ── Départ : carte vide sur l'index ─────────────────────────────────────── */
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}va-carte-index-empty.png` });
  await ctx.close();
}
...
for (const n of [1, 2, 3, 4, 5]) {
  ...
  if (n === 4) {
    ...
    await page.locator('#km-root [data-km-item="faisceau-intervalle"]').click(); await settle(page, 300);
    check('M4: navigation → item detail opens', /Retour à la carte/.test(await body(page)));
  ...
  if (n === 4) {
    ...
    check('M4: print header carries the lesson title, not « LES VECTEURS »', /VALEUR ABSOLUE ET DISTANCE/.test(printText) && !/LES VECTEURS/.test(printText));
    check('M4: print view shows nothing from M5', !/Traduire une tolérance/.test(printText));
    check('M4: print CSS swaps screen → print view', screenHidden && printShown);
    await page.screenshot({ path: `${SHOT_DIR}va-carte-m4-print.png`, fullPage: true });
  ...
  if (n === 5) {
    ...
    await page.screenshot({ path: `${SHOT_DIR}va-carte-m5-final.png`, fullPage: true });
  }
  await ctx.close();
}
...
/* ── Boss : la synthèse EST la carte complète ────────────────────────────── */
{
  ...
  check('boss synthèse: old hand-written synthèse gone', !/Synthèse : le phare/.test(await body(page)));
  ...
  await page.screenshot({ path: `${SHOT_DIR}va-carte-boss.png`, fullPage: true });
  await ctx.close();
}
...
/* ── Mobile ──────────────────────────────────────────────────────────────── */
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}va-carte-mobile.png` });
  await ctx.close();
}

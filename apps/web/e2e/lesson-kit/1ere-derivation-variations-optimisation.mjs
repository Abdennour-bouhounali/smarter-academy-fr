...
const BASE = process.env.KIT_BASE || 'http://localhost:5290';
const LESSON = '/courses/lycee/premiere_specialite/analyse/derivation-variations-optimisation-1ere';
const KEY = 'u_anon_smarter_lesson_derivation-variations-optimisation-1ere';

const M = {
  0: `${BASE}/courses/lycee/premiere_specialite/analyse/derivation-variations-optimisation-1ere/mission-de-depart`,
  1: `${BASE}/courses/lycee/premiere_specialite/analyse/derivation-variations-optimisation-1ere/deux-lignes-qui-se-repondent`,
  2: `${BASE}/courses/lycee/premiere_specialite/analyse/derivation-variations-optimisation-1ere/le-signe-decide-du-sens`,
  3: `${BASE}/courses/lycee/premiere_specialite/analyse/derivation-variations-optimisation-1ere/construire-le-tableau`,
  4: `${BASE}/courses/lycee/premiere_specialite/analyse/derivation-variations-optimisation-1ere/atelier-de-tableaux`,
  5: `${BASE}/courses/lycee/premiere_specialite/analyse/derivation-variations-optimisation-1ere/le-meilleur-choix`,
  6: `${BASE}/courses/lycee/premiere_specialite/analyse/derivation-variations-optimisation-1ere/mission-finale-le-sommet`,
};

/** Apports de chaque module à la carte — miroir de knowledge.jsx. */
const CONTRIB = {
  2: ['signe-derivee-donne-sens', 'mem-positive-monte', 'changement-de-signe-decide'],
  3: ['methode-construire-tableau-depuis-derivee', 'regle-ligne-derivee-au-dessus', 'methode-extremum-par-le-signe', 'mem-plus-moins-maximum'],
  4: ['regle-derivee-jamais-nulle'],
  5: ['methode-optimiser-avec-la-derivee', 'regle-annuler-ne-suffit-pas-optimisation', 'mem-lire-la-reponse-dans-le-tableau'],
};
...
// ── Index ────────────────────────────────────────────────────────────────
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}derivation-variations-optimisation-1ere-index.png`, fullPage: true });
  await ctx.close();
}
...
// ── Modules d'enseignement : manipulables, non gelés, mise en page saine ──
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}derivation-variations-optimisation-1ere-m1.png`, fullPage: true });
  await ctx.close();
}
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}derivation-variations-optimisation-1ere-m2.png`, fullPage: true });
  await ctx.close();
}
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}derivation-variations-optimisation-1ere-m3.png`, fullPage: true });
  await ctx.close();
}
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}derivation-variations-optimisation-1ere-m4.png`, fullPage: true });
  await ctx.close();
}
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}derivation-variations-optimisation-1ere-m5.png`, fullPage: true });
  await ctx.close();
}
...
// ── Le boss ──────────────────────────────────────────────────────────────
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}derivation-variations-optimisation-1ere-boss.png`, fullPage: true });
  await ctx.close();
}
...
for (const n of [1, 6]) {
  ...
  if (n === 1) await page.screenshot({ path: `${SHOT_DIR}derivation-variations-optimisation-1ere-m1-mobile.png`, fullPage: true });
  await ctx.close();
}

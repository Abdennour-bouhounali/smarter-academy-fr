// Carte des connaissances — Logique et raisonnement 2nde : progression cumulative,
// « À retenir » = snapshot de la carte, tiroir = même état, impression, responsive.
// Run: node apps/web/e2e/lesson-kit/2nde-logique-carte.mjs   (vite on :5240, started from apps/web/)
import {
...
const SLUG = {
  1: 'la-formule-qui-tombe', 2: 'et-ou-non', 3: 'implication-et-reciproque',
  4: 'equivalence', 5: 'labsurde',
  boss: 'mission-finale-le-tribunal',
};
...
const CONTRIB = {
  1: ['proposition', 'contre-exemple', 'regle-refuter-prouver', 'mem-contre-exemple'],
  2: ['connecteurs', 'regle-ou-inclusif', 'methode-negation', 'mem-negation-limite'],
  3: ['implication', 'reciproque', 'contraposee', 'mem-cas-interdit'],
  4: ['equivalence', 'regle-equivalence-carre', 'methode-choisir-symbole'],
  5: ['raisonnement-absurde', 'disjonction-cas', 'vocab-quatre-outils'],
};
...
/* ── Départ : carte vide sur l'index ─────────────────────────────────────── */
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}lg-carte-index-empty.png` });
  await ctx.close();
}
...
for (const n of [1, 2, 3, 4, 5]) {
  ...
  if (n === 3) {
    ...
    await page.locator('#km-root [data-km-item="contraposee"]').click(); await settle(page, 300);
  ...
  if (n === 4) {
    ...
    check('M4: print header carries the lesson title, not « LES VECTEURS »', /LOGIQUE ET RAISONNEMENT/.test(printText) && !/LES VECTEURS/.test(printText));
    check('M4: print view shows nothing from M5', !/absurde/i.test(printText));
    check('M4: print CSS swaps screen → print view', screenHidden && printShown);
    await page.screenshot({ path: `${SHOT_DIR}lg-carte-m4-print.png`, fullPage: true });
  ...
  if (n === 5) {
    ...
    await page.screenshot({ path: `${SHOT_DIR}lg-carte-m5-final.png`, fullPage: true });
  }
  await ctx.close();
}
...
/* ── Boss : la synthèse EST la carte complète ────────────────────────────── */
{
  ...
  check('boss synthèse: old hand-written synthèse gone', !/Synthèse : le tribunal/.test(await body(page)));
  ...
  await page.screenshot({ path: `${SHOT_DIR}lg-carte-boss.png`, fullPage: true });
  await ctx.close();
}
...
/* ── Mobile ──────────────────────────────────────────────────────────────── */
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}lg-carte-mobile.png` });
  await ctx.close();
}

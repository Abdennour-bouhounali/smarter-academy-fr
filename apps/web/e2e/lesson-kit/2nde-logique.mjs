...
const M = {
  ...
  m3: `${LESSON}/implication-et-reciproque`, m4: `${LESSON}/equivalence`, m5: `${LESSON}/a-retenir`,
  m6: `${LESSON}/labsurde`, boss: `${LESSON}/mission-finale-le-tribunal`,
};
...
/* M5 */
{
  ...
  for (let i = 0; i < 4; i += 1) await rows.nth(i).locator('button').first().click();
  await settle(page);
  check('M5: card batch reveals', /Contraposée : toujours équivalente/.test(await body(page)));
  await tapOption(page, '#step-2', 1);                                // wrong
  check('M5: « il existe » is proved by one example', /IL EXISTE|un seul n qui convient/.test(await body(page)));
  const rows3 = page.locator('#step-3 div[role="group"]');
  for (let i = 0; i < 3; i += 1) await rows3.nth(i).locator('button').first().click();
  await settle(page);
...
/* M6 — absurd */
{
  ...
  let b = await body(page);
  check('M6: 12 pupils, no share yet', /Douze élèves, douze mois/.test(b));
  await add.click(); await settle(page); await audit(page, issues);
  b = await body(page);
  check('M6: the 13th forces a shared month', /Au 13e élève/.test(b));
  await tapOption(page, '#step-1', 1);                                // wrong
  check('M6: the absurd structure named', /supposé le CONTRAIRE|suppose le contraire/i.test(await body(page)));
  ...
  check('M6: disjunction of cases explained', /deux cas suffisent|couvrent tout/.test(await body(page)));
  check('M6: complete', await nextEnabled(page));
  check('M6: month grid lays out', issues.length === 0, issues.slice(0, 3).join(' | '));
  await page.screenshot({ path: `${SHOT_DIR}lg-m6.png`, fullPage: true });
  await ctx.close();
}
...
/* Boss */
{
  ...
  check('boss: synthèse shows the falling formula', /1 681 = 41 × 41|41 × 41/.test(await body(page)));

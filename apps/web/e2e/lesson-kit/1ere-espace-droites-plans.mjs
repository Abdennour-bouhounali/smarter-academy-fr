/**
 * 1ère spé — « Espace : droites et plans ».
 *
 ...
 *   node apps/web/e2e/lesson-kit/1ere-espace-droites-plans.mjs
 *
...
const LESSON = '/courses/lycee/premiere_specialite/geometrie/espace-droites-plans-1ere';
const KEY = 'u_anon_smarter_lesson_espace-droites-plans-1ere';

const M = {
  0: `${BASE}/courses/lycee/premiere_specialite/geometrie/espace-droites-plans-1ere/mission-de-depart`,
  1: `${BASE}/courses/lycee/premiere_specialite/geometrie/espace-droites-plans-1ere/la-droite-qui-traverse`,
  2: `${BASE}/courses/lycee/premiere_specialite/geometrie/espace-droites-plans-1ere/trois-positions-un-seul-calcul`,
  3: `${BASE}/courses/lycee/premiere_specialite/geometrie/espace-droites-plans-1ere/deux-plans-deux-normaux`,
  4: `${BASE}/courses/lycee/premiere_specialite/geometrie/espace-droites-plans-1ere/le-parametre-qui-parcourt`,
  5: `${BASE}/courses/lycee/premiere_specialite/geometrie/espace-droites-plans-1ere/l-equation-du-plan`,
  6: `${BASE}/courses/lycee/premiere_specialite/geometrie/espace-droites-plans-1ere/demontrer-sans-le-dessin`,
  7: `${BASE}/courses/lycee/premiere_specialite/geometrie/espace-droites-plans-1ere/mesurer-l-ecart`,
  8: `${BASE}/courses/lycee/premiere_specialite/geometrie/espace-droites-plans-1ere/mission-finale-la-boite-percee`,
};

/** Apports de chaque module à la carte — miroir de knowledge.jsx. */
const CONTRIB = {
  1: ['trois-positions-droite-plan', 'un-nombre-decide'],
  2: ['vecteur-normal', 'critere-droite-plan', 'mem-nul-puis-appartenance'],
  3: ['deux-plans-deux-cas', 'critere-deux-plans'],
  4: ['representation-parametrique', 'methode-ecrire-parametrique'],
  5: ['equation-cartesienne-plan', 'mem-abc-est-le-normal', 'methode-equation-plan'],
  6: ['parallelisme-espace-trois-formes', 'orthogonalite-espace-deux-formes', 'mem-le-role-s-inverse'],
  7: ['formule-distance-point-plan', 'methode-distance-espace'],
};
...
// ── Index ────────────────────────────────────────────────────────────────
{
  ...
  check('index : la durée du catalogue est affichée', /85\s*min/.test(txt), txt.slice(0, 160));
  ...
  await page.screenshot({ path: `${SHOT_DIR}espace-droites-plans-1ere-index.png`, fullPage: true });
  await ctx.close();
}
...
// ── Modules d'enseignement : manipulables, non gelés, mise en page saine ──
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}espace-droites-plans-1ere-m1.png`, fullPage: true });
  await ctx.close();
}
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}espace-droites-plans-1ere-m2.png`, fullPage: true });
  await ctx.close();
}
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}espace-droites-plans-1ere-m3.png`, fullPage: true });
  await ctx.close();
}
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}espace-droites-plans-1ere-m4.png`, fullPage: true });
  await ctx.close();
}
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}espace-droites-plans-1ere-m5.png`, fullPage: true });
  await ctx.close();
}
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}espace-droites-plans-1ere-m6.png`, fullPage: true });
  await ctx.close();
}

{
  const { ctx, page } = await o(M[7], { completedModules: seedThrough(6), tag: 'm7' });
  const issues = [];
  const avant = await body(page);
  check('M7 : la page se rend sans NaN ni erreur de rendu', !/NaN|undefined/.test(avant));
  const swept = await sweepAll(page, '#step-1', issues, 5);
  // Une étape se pilote au cliquet (boutons étiquetés) OU par saisie/choix :
  // exiger un cliquet partout serait une hypothèse de gabarit, pas une règle.
  // Un bouton de bascule dont le TEXTE porte le sens (« ⇄ échanger les deux
  ...
  check('M7 : l’étape 1 est réellement interactive', swept > 0 || saisies > 0, `${swept} cliquet(s), ${saisies} saisie(s)`);
  check('M7 : mise en page saine sur tout le balayage', issues.length === 0, issues.slice(0, 3).join(' | '));
  // JAMAIS GELÉ : après usage, l'étape reste PILOTABLE. Un bouton de cliquet
  // peut être légitimement éteint en BUTÉE (« descendre » au cran le plus bas) :
  // c'est une borne, pas un gel. Et depuis la règle « le glisser d'abord », la
  // commande survivante peut n'être AUCUN bouton — juste la poignée qu'on saisit.
  ...
  if (swept > 0) {
    ...
    check('M7 : la manipulation reste utilisable après usage', encore > 0);
  }
  await page.screenshot({ path: `${SHOT_DIR}espace-droites-plans-1ere-m7.png`, fullPage: true });
  await ctx.close();
}
...
// ── Le boss ──────────────────────────────────────────────────────────────
{
  const { ctx, page } = await o(M[8], { completedModules: seedThrough(7), tag: 'boss' });
  ...
  check('boss : le module d’évaluation est enregistré', Array.isArray(done) && done.includes('8'), JSON.stringify(done));
  await page.reload();
  await settle(page, 1200);
  check('boss : après rechargement, la correction est restituée', /\/\s*10/.test(await body(page)));
  await page.screenshot({ path: `${SHOT_DIR}espace-droites-plans-1ere-boss.png`, fullPage: true });
  await ctx.close();
}
...
for (const n of [1, 8]) {
  ...
  if (n === 1) await page.screenshot({ path: `${SHOT_DIR}espace-droites-plans-1ere-m1-mobile.png`, fullPage: true });
  await ctx.close();
}

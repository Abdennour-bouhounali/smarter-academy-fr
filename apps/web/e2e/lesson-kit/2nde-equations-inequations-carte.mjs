// Carte des connaissances — Équations et inéquations 2nde : progression cumulative,
// « À retenir » = snapshot de la carte, tiroir = même état, impression, responsive.
// Run: node apps/web/e2e/lesson-kit/2nde-equations-inequations-carte.mjs   (vite on :5240, started from apps/web/)
import {
...
const SLUG = {
  1: 'le-scanner-de-solutions',
  2: 'isoler-x-sans-casser-legalite',
  3: 'le-signe-qui-se-retourne',
  4: 'produit-nul',
  5: 'quotient-et-valeur-interdite',
  6: 'modeliser',
  boss: 'mission-finale-les-deux-forfaits',
};
...
/* ── Départ : carte vide sur l'index ─────────────────────────────────────── */
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}eq-carte-index-empty.png` });
  await ctx.close();
}
...
for (const n of [1, 2, 3, 4, 5, 6]) {
  ...
  if (n === 4) {
    ...
    await page.locator('#km-root [data-km-item="produit-nul"]').click(); await settle(page, 300);
  ...
  if (n === 5) {
    ...
    check('M5: print header carries the lesson title, not « LES VECTEURS »', /ÉQUATIONS ET INÉQUATIONS/.test(printText) && !/LES VECTEURS/.test(printText));
    check('M5: print view shows nothing from M6', !/Mettre un problème en équation/.test(printText));
    check('M5: print CSS swaps screen → print view', screenHidden && printShown);
    await page.screenshot({ path: `${SHOT_DIR}eq-carte-m5-print.png`, fullPage: true });
  ...
  if (n === 6) {
    ...
    await page.screenshot({ path: `${SHOT_DIR}eq-carte-m6-final.png`, fullPage: true });
  }
  await ctx.close();
}
...
/* ── Boss : la synthèse EST la carte complète ────────────────────────────── */
{
  ...
  check('boss synthèse: old hand-written synthèse gone', !/Synthèse : les deux forfaits/.test(await body(page)));
  ...
  await page.screenshot({ path: `${SHOT_DIR}eq-carte-boss.png`, fullPage: true });
  await ctx.close();
}
...
/* ── Mobile ──────────────────────────────────────────────────────────────── */
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}eq-carte-mobile.png` });
  await ctx.close();
}

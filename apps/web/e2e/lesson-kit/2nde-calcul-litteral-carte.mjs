// Carte des connaissances — Calcul littéral 2nde : progression cumulative,
// « À retenir » = snapshot de la carte, tiroir = même état, impression, responsive.
// Run: node apps/web/e2e/lesson-kit/2nde-calcul-litteral-carte.mjs   (vite on :5240, started from apps/web/)
import {
...
const SLUG = {
  1: 'le-tour-de-magie',
  2: 'reduire',
  3: 'developper-laire-qui-se-decoupe',
  4: 'factoriser-le-facteur-commun',
  5: 'trois-formes-trois-usages',
  6: 'demontrer-et-resoudre',
  boss: 'mission-finale-le-magicien',
};
...
const CONTRIB = {
  1: ['expression-litterale', 'egalite-pour-tout-x', 'regle-tester-ne-prouve-pas'],
  2: ['vocab-terme-coefficient', 'methode-reduire', 'regle-exposants-pas-additionnes', 'mem-reduire'],
  3: ['developper', 'identites-remarquables', 'mem-carre-somme'],
  4: ['factoriser', 'methode-facteur-commun', 'methode-identite-inverse'],
  5: ['regle-choisir-la-forme', 'regle-carre-positif', 'methode-prouver-egalite-formes'],
  6: ['methode-demontrer-litteral', 'methode-modeliser-aire', 'regle-simplifier-facteurs'],
};
...
/* ── Départ : carte vide sur l'index ─────────────────────────────────────── */
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}cl-carte-index-empty.png` });
  await ctx.close();
}
...
for (const n of [1, 2, 3, 4, 5, 6]) {
  ...
  if (n === 3) {
    ...
    await page.locator('#km-root [data-km-item="identites-remarquables"]').click(); await settle(page, 300);
  ...
  if (n === 5) {
    ...
    check('M5: print header carries the lesson title, not « LES VECTEURS »', /CALCUL LITTÉRAL/.test(printText) && !/LES VECTEURS/.test(printText));
    check('M5: print view shows nothing from M6', !/Modéliser une situation/.test(printText));
    check('M5: print CSS swaps screen → print view', screenHidden && printShown);
    await page.screenshot({ path: `${SHOT_DIR}cl-carte-m5-print.png`, fullPage: true });
  ...
  if (n === 6) {
    ...
    await page.screenshot({ path: `${SHOT_DIR}cl-carte-m6-final.png`, fullPage: true });
  }
  await ctx.close();
}
...
/* ── Boss : la synthèse EST la carte complète ────────────────────────────── */
{
  ...
  check('boss synthèse: old hand-written synthèse gone', !/Synthèse : le tour de magie, démontré/.test(await body(page)));
  ...
  await page.screenshot({ path: `${SHOT_DIR}cl-carte-boss.png`, fullPage: true });
  await ctx.close();
}
...
/* ── Mobile ──────────────────────────────────────────────────────────────── */
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}cl-carte-mobile.png` });
  await ctx.close();
}

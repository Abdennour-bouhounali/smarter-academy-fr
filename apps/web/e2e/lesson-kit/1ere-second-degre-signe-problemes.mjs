/**
 * 1ère spé — « Second degré : signe et problèmes ».
 *
 * Vite détaché depuis apps/web/ :
 *   cd apps/web && (setsid nohup npx vite --port 5286 --strictPort > e2e/lesson-kit/shots/vite-5286.log 2>&1 </dev/null &)
 *   node apps/web/e2e/lesson-kit/1ere-second-degre-signe-problemes.mjs
 *
...
const BASE = process.env.KIT_BASE || 'http://localhost:5286';
const LESSON = '/courses/lycee/premiere_specialite/algebre/second-degre-signe-problemes-1ere';
const KEY = 'u_anon_smarter_lesson_second-degre-signe-problemes-1ere';

const M = {
  0: `${BASE}/courses/lycee/premiere_specialite/algebre/second-degre-signe-problemes-1ere/mission-de-depart`,
  1: `${BASE}/courses/lycee/premiere_specialite/algebre/second-degre-signe-problemes-1ere/le-pont-et-la-peniche`,
  2: `${BASE}/courses/lycee/premiere_specialite/algebre/second-degre-signe-problemes-1ere/le-signe-d-un-trinome`,
  3: `${BASE}/courses/lycee/premiere_specialite/algebre/second-degre-signe-problemes-1ere/le-tableau-de-signes`,
  4: `${BASE}/courses/lycee/premiere_specialite/algebre/second-degre-signe-problemes-1ere/resoudre-une-inequation`,
  5: `${BASE}/courses/lycee/premiere_specialite/algebre/second-degre-signe-problemes-1ere/du-probleme-au-modele`,
  6: `${BASE}/courses/lycee/premiere_specialite/algebre/second-degre-signe-problemes-1ere/la-reponse-au-probleme`,
  7: `${BASE}/courses/lycee/premiere_specialite/algebre/second-degre-signe-problemes-1ere/mission-finale-le-signe`,
};

/** Apports de chaque module à la carte — miroir de knowledge.jsx. */
const CONTRIB = {
  1: ['signe-change-aux-racines', 'bande-entre-les-racines'],
  2: ['signe-trinome-regle', 'signe-sans-racine', 'mem-signe-trinome'],
  3: ['tableau-signes-trinome', 'methode-dresser-tableau', 'mem-tableau-signes'],
  4: ['inequation-second-degre', 'methode-inequation-second-degre', 'bornes-incluses-inequation'],
  5: ['modeliser-second-degre', 'traduire-au-moins-au-plus'],
  6: ['solution-equation-vs-probleme', 'methode-interpreter', 'mem-ecarter-solutions'],
};
...
// ── Index ────────────────────────────────────────────────────────────────
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}second-degre-signe-problemes-1ere-index.png`, fullPage: true });
  await ctx.close();
}
...
// ── Modules d'enseignement : manipulables, non gelés, mise en page saine ──
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}second-degre-signe-problemes-1ere-m1.png`, fullPage: true });
  await ctx.close();
}
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}second-degre-signe-problemes-1ere-m2.png`, fullPage: true });
  await ctx.close();
}
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}second-degre-signe-problemes-1ere-m3.png`, fullPage: true });
  await ctx.close();
}
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}second-degre-signe-problemes-1ere-m4.png`, fullPage: true });
  await ctx.close();
}
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}second-degre-signe-problemes-1ere-m5.png`, fullPage: true });
  await ctx.close();
}
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}second-degre-signe-problemes-1ere-m6.png`, fullPage: true });
  await ctx.close();
}
...
// ── Le boss ──────────────────────────────────────────────────────────────
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}second-degre-signe-problemes-1ere-boss.png`, fullPage: true });
  await ctx.close();
}
...
for (const n of [1, 7]) {
  ...
  if (n === 1) await page.screenshot({ path: `${SHOT_DIR}second-degre-signe-problemes-1ere-m1-mobile.png`, fullPage: true });
  await ctx.close();
}

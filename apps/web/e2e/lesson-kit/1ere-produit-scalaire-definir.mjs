/**
 * 1ère spé — « Produit scalaire : définir et détecter l’orthogonalité ».
 *
 * Vite détaché depuis apps/web/ :
 *   cd apps/web && (setsid nohup npx vite --port 5283 --strictPort > e2e/lesson-kit/shots/vite-5283.log 2>&1 </dev/null &)
 *   node apps/web/e2e/lesson-kit/1ere-produit-scalaire-definir.mjs
 *
...
const BASE = process.env.KIT_BASE || 'http://localhost:5283';
const LESSON = '/courses/lycee/premiere_specialite/geometrie/produit-scalaire-definir-1ere';
const KEY = 'u_anon_smarter_lesson_produit-scalaire-definir-1ere';

const M = {
  0: `${BASE}/courses/lycee/premiere_specialite/geometrie/produit-scalaire-definir-1ere/mission-de-depart`,
  1: `${BASE}/courses/lycee/premiere_specialite/geometrie/produit-scalaire-definir-1ere/l-ombre-portee`,
  2: `${BASE}/courses/lycee/premiere_specialite/geometrie/produit-scalaire-definir-1ere/le-nombre-a-deux-visages`,
  3: `${BASE}/courses/lycee/premiere_specialite/geometrie/produit-scalaire-definir-1ere/une-algebre-de-fleches`,
  4: `${BASE}/courses/lycee/premiere_specialite/geometrie/produit-scalaire-definir-1ere/demontrer-l-angle-droit`,
  5: `${BASE}/courses/lycee/premiere_specialite/geometrie/produit-scalaire-definir-1ere/la-fleche-qui-tient-la-droite`,
  6: `${BASE}/courses/lycee/premiere_specialite/geometrie/produit-scalaire-definir-1ere/mission-finale-l-angle-droit`,
};

/** Apports de chaque module à la carte — miroir de knowledge.jsx. */
const CONTRIB = {
  1: ['ombre-signee', 'deux-recettes-un-nombre'],
  2: ['vocab-produit-scalaire', 'formule-coordonnees-scalaire', 'formule-normes-angle', 'methode-choisir-la-formule'],
  3: ['regle-symetrie-scalaire', 'regle-bilinearite', 'regle-carre-scalaire'],
  4: ['regle-orthogonalite', 'mem-nul-donc-droit', 'methode-demontrer-orthogonal'],
  5: ['vocab-vecteur-normal', 'regle-equation-cartesienne', 'methode-equation-par-le-normal', 'mem-abc-normal'],
};
...
// ── Index ────────────────────────────────────────────────────────────────
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}produit-scalaire-definir-1ere-index.png`, fullPage: true });
  await ctx.close();
}
...
// ── Modules d'enseignement : manipulables, non gelés, mise en page saine ──
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}produit-scalaire-definir-1ere-m1.png`, fullPage: true });
  await ctx.close();
}
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}produit-scalaire-definir-1ere-m2.png`, fullPage: true });
  await ctx.close();
}
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}produit-scalaire-definir-1ere-m3.png`, fullPage: true });
  await ctx.close();
}
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}produit-scalaire-definir-1ere-m4.png`, fullPage: true });
  await ctx.close();
}
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}produit-scalaire-definir-1ere-m5.png`, fullPage: true });
  await ctx.close();
}
...
// ── Le boss ──────────────────────────────────────────────────────────────
{
  ...
  await page.screenshot({ path: `${SHOT_DIR}produit-scalaire-definir-1ere-boss.png`, fullPage: true });
  await ctx.close();
}
...
for (const n of [1, 6]) {
  ...
  if (n === 1) await page.screenshot({ path: `${SHOT_DIR}produit-scalaire-definir-1ere-m1-mobile.png`, fullPage: true });
  await ctx.close();
}
